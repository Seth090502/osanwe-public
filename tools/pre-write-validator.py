...
EXEMPT_PATH_PREFIXES = (
    "wiki/maintenance/",
    # "Calendar/decisions/" REMOVED 2026-08-12. It exempted the vault's three most
    # important append-only ledgers from every GATE check, and combined with
    # guard-paths.sh never covering that directory it meant sessions-log.md,
    # decision-log.md and execute-or-decline.md had NO mechanical write protection at
    # all -- the append-only rule was instruction-only. tools/append-only-check.py now
    # enforces the strict-prefix rule PreToolUse (registered ahead of this validator),
    # and the ledgers get normal GATE validation like every other vault file.
    # Gate: wiki/research/gates/gate-b-append-only-ledger-guard-2026-08-12.md
)
...
def main():
    ...
    fp = Path(file_path)
    # Only markdown is validated. .base files are Obsidian Bases YAML view configs --
    # they carry no `---` markdown frontmatter and no [[wikilinks]], so routing them
    # through the .md GATE audit only ever fires a missing-frontmatter FALSE POSITIVE
    # (S5; tenfold-t10). .base structure is validated separately by the /vault X60
    # classifier. This aligns the code with the docstring's stated skip contract.
    if fp.suffix != ".md":
        sys.exit(0)
    ...
    # Bare README.md files are documentation scaffolding, not vault notes -- they carry
    # no canonical frontmatter (this generalizes the per-file README entries already in
    # EXEMPT_PATHS so a new README never trips the missing-frontmatter GATE). S10; t10.
    # (This alone fixes the reported data/README.md FP; the non-vault data/ dir is also
    # in EXEMPT_DIRS. A blanket gitignore skip was rejected: it would also skip the
    # gitignored test-tmp/ vault scratch the prevention-arch harness validates.)
