...
Coverage matrix:
  ...
  T10 SPLT split preservation: extended_hours_last reflects post-split price
...
Run: py tools/test-fetch-prices.py
"""
...
class TestExtendedHoursDiffusion(unittest.TestCase):
    ...
    def test_T5_subagent_contract_additivity(self):
        ...
        Reads the SOURCE under .agents/roles/, not the generated copy under
        .claude/agents/. gen-roles.py writes the latter from the former, so the
        source is where the contract actually lives; asserting against the
        generated copy meant this test failed the moment the duplicate stopped
        being carried, which is exactly what it should not care about. Falls back
        to the generated copy so the test still runs in a tree that has one and
        no source.
        """
        root = Path(__file__).parent.parent
        agent_path = root / ".agents" / "roles" / "price-fetcher.md"
        if not agent_path.is_file():
            agent_path = root / ".claude" / "agents" / "price-fetcher.md"
        self.assertTrue(agent_path.is_file(),
                        "T5: no price-fetcher definition found under .agents/roles/ "
                        "or .claude/agents/")
    ...
    def test_T10_split_preservation(self):
        """T10: SPLT split passed through via auto_adjust=True (default).
        The mock provides post-split prices; AH extraction should report them as-is.
        """
        today_et = datetime.now(ET).date()
        ts = datetime(today_et.year, today_et.month, today_et.day, 19, 0, tzinfo=ET).astimezone(timezone.utc)
        # Post-split SPLT price ~$80 (was ~$640 pre-split)
        bars = [(ts.isoformat(), 80.0, 80.5, 79.8, 80.2, 100000)]

        all_tickers = ['SPLT'] + list(fp.INDEX_MAP.keys())
        daily = make_daily_df(all_tickers, base_price=80.0)
        intraday = make_intraday_df({t: bars if t == 'SPLT' else [] for t in all_tickers})

        fake_now = datetime(today_et.year, today_et.month, today_et.day, 19, 5, tzinfo=ET)
        result = self._patched_fetch_all(['SPLT'], [], daily, intraday, fake_now_et=fake_now)

        splt = result['equities']['SPLT']
        # Assert post-split price reflected; not normalized to pre-split
        self.assertIsNotNone(splt['extended_hours_last'])
        self.assertLess(splt['extended_hours_last'], 100.0,
                        "T10: post-split SPLT should be ~$80; not pre-split ~$640")
