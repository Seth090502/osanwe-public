...
ACTIVE_CONTEXT_V2_SUBSECTIONS = {
    ...
    "### Active Blockers",      # NEW -- /retro+/invest
...
def _hook_skip_non_hot_md() -> bool:
    """When invoked as a PostToolUse hook, Claude Code passes the tool call as
    JSON on stdin. The hook fires on EVERY Write/Edit/MultiEdit, but this
    validator only concerns wiki/hot.md. Return True (skip) when the hook JSON
    identifies a written file that is not wiki/hot.md. Return False for CLI
    invocations (no JSON on stdin, e.g. /retro and /vault direct calls) or when
    the write targets hot.md, so direct callers are unaffected."""
    try:
    ...
    if not fp:
        return False  # hook JSON without a path -- be conservative, run the check
    return not fp.replace("\\", "/").lower().endswith("wiki/hot.md")
...
def main() -> int:
    ...
    # PostToolUse hook fires on every Write/Edit/MultiEdit; this validator only
    # concerns wiki/hot.md. Skip immediately for writes to any other file.
    if _hook_skip_non_hot_md():
        return 0
