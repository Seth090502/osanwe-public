...
# -------------------------------------------------------------------
Step 5 "Best-effort: add /path/to/vault to claudewatch scan paths (SOFT, non-fatal)"
# Note: VaultRoot sessions are ALREADY covered via ~/.claude/projects transcripts.
...
if ($DryRun) {
    ...
    if (Test-Path $ConfigYaml) {
        ...
        if (($yaml -match '/path/to/vault') -or ($yaml -match '/path/to/vault')) {
            ...
            Warn ("no scan_paths key in config.yaml; not guessing schema. VaultRoot is still covered via ~/.claude transcripts. To add project-health coverage, edit " + $ConfigYaml + " manually (scan_paths list).")
        }
    } else {
        Warn ("config.yaml not present yet (claudewatch may create it lazily on first metric query). VaultRoot still covered via ~/.claude transcripts. Re-run this script after first claudewatch use to patch scan_paths, or add it manually.")
    }
}
