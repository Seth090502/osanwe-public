...
#   T5  pre-write-validator: EXEMPT_PATHS (AGENTS.md) -> exit 0
...
#   T20 vault-audit.py X3: contract count-drift classifier emits valid schema
#   T21 vault-audit.py X5: knowledge-moc count-drift classifier emits valid schema
#   T22 pre-write-validator: Calendar/decisions/ gets normal GATE validation (SD-PV1 exemption removed 2026-08-12)
...
# D54: this suite never runs against the tree it lives in. It used to hard-code
# VAULT_ROOT="/path/to/vault" and write its fixtures into the live vault, and the
# hooks it exercises hard-code that root too, so every checkall.py run of a
# manifest check wrote into the vault it was checking. Unless it is already
# inside a throwaway copy, it now builds one (tools/hook_suite_copy.py: tracked
# files only, never the private areas, every hard-coded root in the COPY's hook
# code pointed at the copy), re-runs itself there, and removes the copy. The
# tree it was started from is only read. Copy mode needs the variable AND the
# marker file the copier writes, so a stray variable cannot make it run in place.
if [ -n "${OSANWE_HOOK_SUITE_COPY:-}" ] && \
        [ ! -f "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/.osanwe-hook-suite-copy" ]; then
    echo "test-prevention-arch: OSANWE_HOOK_SUITE_COPY is set but this tree is not a throwaway copy; refusing to run in place" >&2
    exit 1
fi
if [ -z "${OSANWE_HOOK_SUITE_COPY:-}" ]; then
    _src="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && (pwd -W 2>/dev/null || pwd))"
    _tmp="$(mktemp -d "${TMPDIR:-/tmp}/osanwe-hook-suite-XXXXXX")" || {
        echo "test-prevention-arch: no temporary directory; refusing to run against the live tree" >&2
        exit 1
    }
    if ! python "$_src/tools/hook_suite_copy.py" "$_src" "$_tmp/tree" >/dev/null; then
        rm -rf "$_tmp"
        echo "test-prevention-arch: could not build the throwaway copy; refusing to run against the live tree" >&2
        exit 1
    fi
    (
        cd "$_tmp/tree" || exit 1
        _copy="$(pwd -W 2>/dev/null || pwd)"
        OSANWE_HOOK_SUITE_COPY=1 OSANWE_VAULT_ROOT="$_copy" CLAUDE_PROJECT_DIR="$_copy" \
            bash "$_tmp/tree/tools/test-prevention-arch.sh" "$@"
    )
    _rc=$?
    rm -rf "$_tmp"
    exit $_rc
...
# Windows-native paths so both Git Bash and Python interpret them correctly.
# Git Bash auto-translates C:/ to /c/ for shell ops; Python on Windows needs C:/.
# Inside the copy, the root is wherever this script is.
VAULT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && (pwd -W 2>/dev/null || pwd))"
...
data = {
    'tool_name': 'Write',
    'tool_input': {
        'file_path': r'$TEST_DIR/t1-new.md',
...
"
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t1.json" > /dev/null 2>&1
    actual=$?
    set -e
...
data = {
    'tool_name': 'Write',
    'tool_input': {
        'file_path': r'$TEST_DIR/t2-new.md',
...
"
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t2.json" > /dev/null 2>&1
    actual=$?
    set -e
...
data = {
    'tool_name': 'Edit',
    'tool_input': {
        'file_path': r'$TEST_DIR/existing.md',
...
"
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t3.json" > /dev/null 2>&1
    actual=$?
    set -e
...
"
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t4.json" > /dev/null 2>&1
    actual=$?
    set -e
...
data = {
    'tool_name': 'Edit',
    'tool_input': {
        'file_path': r'$VAULT_ROOT/AGENTS.md',
        'old_string': 'foo',
...
"
    ...
    actual=$?
    set -e
    run_test T5 "pre-write-validator: EXEMPT_PATHS AGENTS.md (broken edit allowed)" "$actual" 0
fi
...
with open(r'$JSON_DIR/t7.json', 'w') as f:
    json.dump(data, f)
"
    if [ -n "${OSANWE_HOOK_SUITE_COPY:-}" ]; then
        # wikilink-check gives vault-audit 15 s and lets the write through if it runs out
        # (D78). The first read of a freshly written copy is slow -- 17.8 s measured for
        # the scoped audit, against 12.8-14.1 s on the warm live vault and 10.6 s here
        # once warm -- so read the Markdown once first, as the live tree already has been.
        python -c "
import os
for dp, dn, fn in os.walk(r'$VAULT_ROOT'):
    if '.git' in dn:
        dn.remove('.git')
    for f in fn:
        if f.endswith('.md'):
...
data = {
    'tool_name': 'Write',
    'tool_input': {
        'file_path': r'$TEST_DIR/t16-new.md',
...
"
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t16.json" > /dev/null 2>&1
    actual=$?
    set -e
...
# ---------- T20: vault-audit.py X3 classifier (contract count-drift) ----------
# Audit 2026-04-28 5th-pass X3: Classifier 12 reports root-contract count claims
...
" > /dev/null 2>&1
    actual=$?
    set -e
    run_test T20 "vault-audit.py X3: contract count-drift classifier emits valid schema" "$actual" 0
fi
...
# (investing-moc entity-note + analysis counts; <private-file> company-entity count)
...
# ---------- T22: pre-write-validator gives Calendar/decisions/ normal GATE validation ----------
# The Calendar/decisions/ exemption this test was written for (SD-PV1, 2026-04-28) was
# REMOVED on 2026-08-12 -- see the comment at EXEMPT_PATH_PREFIXES in
# tools/pre-write-validator.py: the ledgers get normal GATE validation now, and
# tools/append-only-check.py enforces their append-only rule. The old T22 still expected
# the exemption and could not fail: its Edit never applied (its old_string is not in the
# ledger), so the validator checked the unchanged ledger, which validates only where the
# private notes it links to exist -- in the live vault, never in this copy (D54). This T22
# edits a small synthetic file in the copy's Calendar/decisions/: a broken forward
# reference must be blocked (exit 2) and the same edit with a resolvable link allowed
# (exit 0). The timeout is raised through the validator's own tuning variable because a
# cold copy is slow to audit; the decision under test is unchanged.
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T22" ]; then
    T22_FILE="$VAULT_ROOT/Calendar/decisions/t22-validator-fixture.md"
    T2X_PATHS_TO_REMOVE+=("$T22_FILE")
    write_fixture "$T22_FILE" << 'EOFT22'
---
aliases: []
...
placeholder-old
EOFT22
    for t22_case in broken resolvable; do
        if [ "$t22_case" = "broken" ]; then t22_link='[[t22-not-yet-existent-target-XYZ]]'; else t22_link='[[hot]]'; fi
        python -c "
import json
data = {
    'tool_name': 'Edit',
    'tool_input': {
        'file_path': r'$T22_FILE',
        'old_string': 'placeholder-old',
        'new_string': 'Modified: $t22_link'
    }
}
with open(r'$JSON_DIR/t22-$t22_case.json', 'w') as f:
    json.dump(data, f)
"
    done
    set +e
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t22-broken.json" > /dev/null 2>&1
    t22_broken=$?
    CLAUDE_PRE_WRITE_TIMEOUT_SEC=90 python "$VAULT_ROOT/tools/pre-write-validator.py" < "$JSON_DIR/t22-resolvable.json" > /dev/null 2>&1
    t22_ok=$?
    rm -f "$T22_FILE"
    set -e
    actual=1
    [ "$t22_broken" = "2" ] && [ "$t22_ok" = "0" ] && actual=0
    run_test T22 "pre-write-validator: Calendar/decisions/ gets normal GATE validation (broken=$t22_broken want 2, resolvable=$t22_ok want 0)" "$actual" 0
fi
...
# 11 tests pass. Invokes via py (python launcher); output captured + suppressed
...
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T28" ]; then
    actual=0
    py "$VAULT_ROOT/tools/test-fetch-prices.py" >"$TEST_DIR/t28-output.log" 2>&1 || actual=1
...
" 2>/dev/null || echo 0
    ...
    set +e
    rel_broken="${T32_BROKEN#"$VAULT_ROOT"/}"
    rel_clean="${T32_CLEAN#"$VAULT_ROOT"/}"
...
# ---------- T39/T40: pretrade-token-gate matcher widening (cross-harness S4) ----------
# The settings.json matcher was widened to mcp__robinhood-trading__.* so the hook's
# defense-in-depth branches (option/mutator outright-block) are REACHABLE (they were
# dead code under the old (place|cancel|review)_equity_order matcher). T39 asserts the
# newly-covered exercise mutator BLOCKS (exit 2); T40 asserts a read tool still flows
# (exit 0) so the widened matcher cannot break the read path.
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T39" ]; then
    set +e
    printf '{"tool_name":"mcp__robinhood-trading__exercise_option","tool_input":{}}' \
        | python "$VAULT_ROOT/.claude/hooks/pretrade-token-gate.py" >/dev/null 2>&1
    actual=$?
    set -e
    run_test T39 "pretrade-token-gate blocks exercise_option (mutator branch reachable, exit 2)" "$actual" 2
fi
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T40" ]; then
    set +e
    printf '{"tool_name":"mcp__robinhood-trading__get_equity_quotes","tool_input":{"symbols":["NVDA"]}}' \
        | python "$VAULT_ROOT/.claude/hooks/pretrade-token-gate.py" >/dev/null 2>&1
    actual=$?
    set -e
    run_test T40 "pretrade-token-gate allows read tools under the widened matcher (exit 0)" "$actual" 0
fi

# ---------- T42: pretrade default-BLOCK on unknown robinhood tool names (redteam H3) ----------
# A NEW server-side tool (surface growth) must fail CLOSED at the hook, not fall
# through to the permission prompt. Also asserts a known read tool still flows.
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T42" ]; then
    set +e
    printf '{"tool_name":"mcp__robinhood-trading__place_crypto_order","tool_input":{}}' \
        | python "$VAULT_ROOT/.claude/hooks/pretrade-token-gate.py" >/dev/null 2>&1
    a1=$?
    printf '{"tool_name":"mcp__robinhood-trading__get_portfolio","tool_input":{}}' \
        | python "$VAULT_ROOT/.claude/hooks/pretrade-token-gate.py" >/dev/null 2>&1
    a2=$?
    set -e
    actual=1
    [ "$a1" = "2" ] && [ "$a2" = "0" ] && actual=0
    run_test T42 "pretrade default-BLOCKs unknown robinhood tool (exit 2) while read set flows (exit 0)" "$actual" 0
fi

# ---------- T41: X26 guard-glob extension covers .agents/mcp (D11, cross-harness S2) ----------
# An UNARMED tool-layer write to .agents/mcp/servers.json must block (exit 2). The
# real arm flag, if present, is moved aside and restored so the test never consumes it.
if [ "$SELECT" = "all" ] || [ "$SELECT" = "T41" ]; then
    ARM_FLAG="$VAULT_ROOT/.claude/state/config-edit-armed"
    ARM_SAVED=0
    [ -f "$ARM_FLAG" ] && { mv "$ARM_FLAG" "$ARM_FLAG.t41-saved"; ARM_SAVED=1; }
    set +e
    printf '{"tool_input":{"file_path":"%s/.agents/mcp/servers.json"}}' "$VAULT_ROOT" \
        | bash "$VAULT_ROOT/.claude/hooks/guard-paths.sh" >/dev/null 2>&1
    actual=$?
    set -e
    [ "$ARM_SAVED" = "1" ] && mv "$ARM_FLAG.t41-saved" "$ARM_FLAG"
    run_test T41 "guard-paths blocks unarmed write to .agents/mcp/servers.json (D11 extension, exit 2)" "$actual" 2
fi
