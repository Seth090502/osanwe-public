...
# OSANWE-V2 SOTA pass: self-healing interpreter. This module needs pandas/yfinance;
# some harnesses (Hermes venv) lack them. Re-exec once under the repo toolchain
# rather than failing -- same pattern as tools/backtest-prediction.py.
def _ensure_market_deps():
    try:
        import pandas  # noqa: F401
    except ImportError:
        for _cand in (r"/path/to/python\python.exe",):
            if os.path.isfile(_cand) and os.path.abspath(sys.executable).lower() != os.path.abspath(_cand).lower():
...
def fetch_all(
    ...
    last_date = None
    equity_data_dates: list[date] = []  # non-crypto close-dates -> consensus last_trading_day
    ...
    for ticker in all_tickers:
        try:
            ...
            # last_date drives last_trading_day + market_status, both of which describe
            # the EQUITY session -- crypto trades 24/7 so its data_date is always today
            # and must not contaminate them (S6 holiday-weekend bug: crypto's Sat date
            # made last_trading_day=Sat 07-04 and forced market_status=open). Collect
            # non-crypto dates; the consensus (mode) is taken after the loop so one
            # yfinance phantom bar cannot inflate last_trading_day. tenfold-t10.
            if not ticker.endswith("-USD"):
                equity_data_dates.append(data_date)
            ...
            # Crypto session is always "regular" (24/7); excluding it from the global
            # reduction keeps market_status an EQUITY-market signal so a weekend/holiday
            # is not reported "open" (S6; tenfold-t10). The per-quote market_session
            # (crypto = "regular") in the entry below is unaffected.
            if not ticker.endswith("-USD"):
    ...
    # Consensus last trading day: the mode of equity/index close-dates, robust to a
    # lone yfinance phantom bar (e.g. a spurious 07-03 bar when NYSE was closed for
    # the observed holiday). On a tie, prefer the OLDER date -- phantoms are spurious
    # NEWER bars on closed days, so max() would over-count the true session. (S6; t10.)
    if equity_data_dates:
        counts = Counter(equity_data_dates)
        top_n = counts.most_common(1)[0][1]
        last_date = min(d for d, c in counts.items() if c == top_n)
