...
Scans all .md files in /path/to/vault/ and outputs a structured markdown report
...
Usage:
    python /path/to/vault\tools\vault-audit.py
    python /path/to/vault\tools\vault-audit.py --scope wiki/research/foo.md --quick
    python /path/to/vault\tools\vault-audit.py --changed-only --json
"""
...
GLOBAL_EXCLUDES = {
    ...
    ".smart-env",
    ".claude",
    ".agents",   # canon agent runtime (skills/scripts/mcp/hooks); not vault content -- .claude/skills is the derived tree this script indexes
    ".codex",    # Mission Four: Codex agents/config/hooks artifacts (TOML; not vault content)
    "skeleton",  # genesis-skeleton nested code repo (gitignored, own .git; 2026-07-07; nested-repo precedent)
    "relay-cases",   # tools/relay-cases: pinned suite fixtures carry PLANTED defects by design (2026-08-17)
    "parity-cases",  # tools/parity-cases: parity fixtures ditto (planted domain:/status:/non-ASCII defects are the test corpus, not vault drift)
    ".claude-ox",    # Ox Alpha experiment runtime tree (2026-08-21): plugin cache + profile, NOT vault content; move to _archive/2026-08-overhaul pending process close (locked live at W2; OSANWE-V2 F-list)
    "_improvement-night",  # archived stalled-run artifacts (moved to _archive/2026-08-overhaul, OSANWE-V2 W2)
}

# Additional excludes for orphan detection (files here aren't expected to have backlinks)
ORPHAN_EXCLUDES = {
    "test-tmp",       # wiki/research/test-tmp: pinned test fixtures + live .precheck staging (OSANWE-V2 ADR-10; standalone by design)
    "shadow",         # test-tmp/shadow: shadow-verdict fixtures ditto
    "drafts",         # wiki/research/drafts: pre-publication drafts; published FINALs live in Atlas/sources (draft->published link kept in-file)
    "_archive",       # archived material never needs backlinks (global _archive dir + wiki/_archive)
    "playbooks",      # wiki/playbooks: consolidated lessons surfaced by search on error strings, not linked browsing (OSANWE-V2 disposition)
    "<private-file>.local.md",   # operator-owned local archive; agents never read it (HB-04) -- no link surface possible
    "OSANWE-<private-file>.local.md",  # ditto
    "templates",
    ".checkpoints",
    "node_modules",
    "nested-tooling",
    ...
    "applications",   # dated sub-files (only the parent file is linked)
...
# Forbidden frontmatter fields per the root contract AGENTS.md (frontmatter schema).
...
# X37/X72 (tenfold-t10, 2026-07-05): archival-by-design paths where a stale
# `updated:` date is CORRECT, not decay -- dated, new-file-per-run, or
# append-and-freeze outputs that the root contract's archival doctrine says are never
# maintained after creation (analyses, research dumps, daily notes, dated decision
# records, briefings, maintenance reports). These land in stale_archival and are
# EXCLUDED from the HARD stale count so the honesty metric (uncapped-HARD) tracks
# REAL decay -- living files that should stay current (Atlas refs, MOCs, entity
# notes, <private-area>, tracker) -- not vault growth. X72 cross-check: the set is
# deliberately CONSERVATIVE; it must never swallow package-confirmed real decay
# (e.g. one private living area stays IN the count -- a long-running private-data
# decay the 2026-06-22 remediation package confirmed is real, not archival).
ARCHIVAL_STALE_EXEMPT_PREFIXES = (
    "wiki/research/",                        # dated dumps/sparks/challenges/prompts/ingest (Pattern 21)
    "wiki/investing/analyses/",             # new-file-per-run, never overwritten
    "wiki/investing/snapshots/",            # dated networth snapshots
    "Calendar/daily/",                      # never archived, never updated post-day
    "Calendar/weekly/",                     # dated weekly reviews
    "Calendar/decisions/briefings/",        # dated PDB-style briefings
    "wiki/maintenance/",                    # dated audit/stats/repair/refresh reports
    "wiki/_archive/", "_archive/",          # explicitly archived
    "<private-area>/applications/",  # dated private records (archival)
    "<private-area>/archive/",
    "<private-area>/scans/",
    "<private-area>/retros/",
)
# The two append-only LIVING logs are never exempt -- they stay current by
# construction (a stale one WOULD be a real signal). Everything else matching
# Calendar/decisions/decision-* is a dated, frozen decision RECORD (archival).
ARCHIVAL_STALE_LIVING_LOGS = {
    "Calendar/decisions/decision-log.md",
    "Calendar/decisions/sessions-log.md",
}


def is_archival_stale_exempt(rel_path: str) -> bool:
    """True if a stale `updated:` on this path is archival-by-design, not decay."""
    rp = rel_path.replace("\\", "/")
    if rp in ARCHIVAL_STALE_LIVING_LOGS:
        return False
    if rp.startswith(ARCHIVAL_STALE_EXEMPT_PREFIXES):
        return True
    if rp.startswith("Calendar/decisions/decision-"):
        return True  # dated decision record (decision-<slug>-<date>.md)
    return False


# X2 (audit 2026-04-28): mechanical body-length cap per the root contract's guideline.
...
def collect_md_files() -> list[Path]:
    ...
    for f in VAULT_ROOT.rglob("*.md"):
        if f.is_file() and not should_exclude(f):
            # mission scratch dirs (Efforts/*/_work/) hold intermediate evidence,
            # not vault organs -- exempt by name (OSANWE-V2, mirrors the pre-commit
            # cage's R6 /_work/ exemption; A4.3 designates these for raw artifacts)
            if "_work" in f.parts:
                continue
            files.append(f)
    return sorted(files)
...
def collect_skill_companion_refs() -> list[Path]:
    ...
    These are referenceable from vault content (e.g., 'related: <private-file>')
...
def detect_forbidden_frontmatter(text: str) -> list[str]:
    ...
    Detects:
    - `domain:` field (forbidden per the AGENTS.md schema)
...
def collect_skill_body_length_violations() -> list[dict]:
    ...
    exceeding SKILL_BODY_LENGTH_CAP (263 lines per the root contract).
...
def collect_claude_md_count_drift() -> list[dict]:
    """X3 Classifier 12 (audit 2026-04-28 5th-pass): contract count-drift detection.

    Reads the root contract, which is AGENTS.md since the CLAUDE.md mirror was
    removed. The JSON key stays claude_md_count_drift so recorded audit artifacts
    and the prevention harness keep reading the same field.

    Parses three count claims in the contract and reconciles against actuals:
    ...
    """
    drifts = []
    contract_path = VAULT_ROOT / "AGENTS.md"
    text = read_file(contract_path)
    if text is None:
        return []
    ...
    if m:
        ...
        if claimed != actual_skill_count:
            drifts.append({
                "location": "AGENTS.md preamble 'all N skills'",
                "claimed": claimed,
    ...
    if table_match:
        ...
        if len(rows) != actual_skill_count:
            drifts.append({
                "location": "AGENTS.md Active Skills table",
                "claimed": len(rows),
                "actual": actual_skill_count,
            })
    ...
    if m:
        ...
        if claimed != actual_investing_refs:
            drifts.append({
                "location": "AGENTS.md Investing ref-doc count",
                "claimed": claimed,
...
def collect_knowledge_moc_count_drift() -> list[dict]:
    ...
    against actuals:
      ...
      - <private-file> "<K> company entities" (only fires drift when claimed > actual)
    ...
    # Parse <private-file> claim (company entities)
    m = re.search(r"<private-file>.*?(\d+) company entities", text)
    if m:
        claimed_c = int(m.group(1))
        # Tolerance: company entities span more than one namespace; only
        ...
        if claimed_c > actual_companies:
            drifts.append({
                "location": "knowledge-moc <private-file> company-entity count",
                "claimed": claimed_c,
...
def resolve_audit_scope(scope: str | None) -> Path | None:
    """Normalize the role's full-vault sentinel and reject unobserved paths."""
    if scope is None or scope.strip().casefold() == "all":
        return None
    if not scope.strip():
        raise ValueError("audit scope cannot be empty")
    path = (VAULT_ROOT / scope).resolve()
    if not path.exists():
        raise ValueError("audit scope does not exist: " + str(path))
    if not (path.is_file() or path.is_dir()):
        raise ValueError("audit scope must be a file or directory")
    return path


def run_audit(scope: str | None = None, changed_only: bool = False,
              quick: bool = False, json_output: bool = False):
    scope_path = resolve_audit_scope(scope)
    today = date.today()
    ...
    # OSANWE-V2: mission scratch wave analyses are wikilink targets from entity
    # notes (graph wiring, 2026-08-24) -- resolve them without auditing _work/
    wave_refs = sorted((VAULT_ROOT / "Efforts/osanwe-v2-overhaul/_work").glob("wave*-analysis.md"))
    extra_targets = skill_refs + base_files + list(wave_refs)
    ...
    needs_global_pass = scope_path is not None or changed_only
    ...
    if scope_path is not None:
        if scope_path.is_file():
            ...
            files = [f for f in files if f.resolve().is_relative_to(scope_path)]
        if not files:
            raise ValueError("audit scope contains no auditable Markdown: " + str(scope_path))
    if not files and not changed_only:
        raise ValueError("full audit contains no auditable Markdown files")
    if changed_only:
    ...
    stale_files: list[tuple[Path, str, int]] = []            # (file, updated_str, days) -- LIVING (real decay; HARD)
    stale_archival: list[tuple[Path, str, int]] = []         # (file, updated_str, days) -- archival-by-design (X37; NOT HARD)
    ...
    for f in files:
        ...
        # Stale detection (X37/X72 tenfold-t10): archival-by-design paths are
        # dated-and-frozen by doctrine -- a stale `updated:` there is correct, not
        # decay -- so they land in stale_archival (reported, not counted). Living
        # files stay in stale_files as REAL decay feeding the HARD count.
        if updated_val:
            d = parse_date(updated_val)
            if d:
                delta = (today - d).days
                if delta > STALE_THRESHOLD_DAYS:
                    if is_archival_stale_exempt(rel_path):
                        stale_archival.append((f, updated_val, delta))
                    else:
        ...
        # per the AGENTS.md schema. GATE-class because schema violations are mechanically
    ...
    w(f"| Stale files (>{STALE_THRESHOLD_DAYS}d, real decay) | {len(stale_files)} |")
    w(f"| Stale files (archival-exempt, not decay) | {len(stale_archival)} |")
    w()
    ...
    # Stale files (real decay -- archival-by-design excluded per X37/X72, see honesty line)
    w("## Stale Files (real decay)")
    w()
    w(f"_{len(stale_archival)} archival-by-design stale files (dated analyses / research "
      f"/ daily notes / decision records / maintenance reports) are excluded here -- a "
      f"stale `updated:` on those is correct, not decay (X37). Living files below should "
      f"stay current; see the UNCAPPED-HARD honesty line._")
    w()
    if stale_files:
        w(f"Found **{len(stale_files)}** living files with `updated` date older than {STALE_THRESHOLD_DAYS} days.")
        w()
    ...
    w()
    # === UNCAPPED-HARD honesty line (X37/X72 tenfold-t10) ===
    # HARD penalty caps at -5, so the score alone can read clean while real drift
    # accumulates. Surface the UNCAPPED count (never capped) as the honesty trend
    # metric, and the archival-exempt count for transparency -- the exclusion is
    # auditable and must never become a way to hide real decay (X72).
    w(f"**Honesty -- UNCAPPED HARD (real decay): {HARD_DRIFT_FINDINGS}** "
      f"(orphans {len(orphans)} + living-stale {len(stale_files)} + session-gaps "
      f"{len(session_artifact_gaps)} + skill-len {len(skill_body_length_violations)}). "
      f"The count is NOT capped; only the -{abs(HARD_CAP)} penalty is. "
      f"Trend target: falling (tenfold-t10 baseline; raw pre-t10 was 368 archival-inclusive).")
    w(f"Archival-exempt (dated-by-design, not decay -- excluded from HARD per X37): "
      f"**{len(stale_archival)}** stale files "
      f"(analyses / research / daily / dated-decisions / maintenance). "
      f"Living paths (Atlas, entities, <private-area>, tracker) stay counted (X72).")
    w()
    ...
    if json_output:
        findings = {
            "scope": "all" if scope_path is None else relative_path(scope_path),
            ...
            # X37/X72 tenfold-t10: archival-by-design stale files (dated-and-frozen;
            # NOT counted as HARD decay). Reported for transparency so the exclusion
            # stays auditable and never hides real decay.
            "stale_archival_exempt": [
                {"file": relative_path(f), "updated": u, "days": d}
                for (f, u, d) in stale_archival
            ],
            # X37/X72 tenfold-t10: uncapped-HARD honesty metric (the count the -5
            # penalty cap conceals). real_decay = the HARD count feeding the score.
            "honesty": {
                "uncapped_hard_real_decay": HARD_DRIFT_FINDINGS,
                "components": {
                    "orphans": len(orphans),
                    "living_stale": len(stale_files),
                    "session_artifact_gaps": len(session_artifact_gaps),
                    "skill_body_length": len(skill_body_length_violations),
                },
                "archival_exempt_stale": len(stale_archival),
                "hard_penalty_cap": HARD_CAP,
            },
            ...
            # X3 audit-2026-04-28 5th-pass: contract count-drift classifier (SOFT)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scope", default=None,
                        help="Existing file/subtree relative to vault root; all or omitted scans the full vault")
    ...
    try:
        ...
        if args.json_output:
            print(json.dumps({"status": "refused", "score": None, "error": str(exc)}))
            raise SystemExit(2)
