...
# full pass -- lands an F11 atomic commit of the vault files. The C--vault-rebuild
...
CONTRACT_MD = VAULT / "AGENTS.md"
SKILL_MD = VAULT / ".agents" / "skills" / "telemetry" / "SKILL.md"   # canon (cross-harness cutover 2026-08-10)
SKILL_MIRROR = VAULT / ".claude" / "skills" / "telemetry" / "SKILL.md"  # sync-generated derived copy
...
Two complementary layers (claudewatch never writes inside /path/to/vault):
...
INDEX_TOOLS_OLD = "## VaultRoot Vault -- Tools (23)"
INDEX_TOOLS_NEW = "## VaultRoot Vault -- Tools (24)"
INDEX_PLAYBOOKS_ANCHOR = "\n\n## VaultRoot Vault -- Playbooks (8)"
...
**Where it lives.** Binary `/path/to/home\.local\bin\claudewatch.exe` (v0.15.0 prebuilt windows_amd64). DB + config out-of-tree at `~/.config/claudewatch/` (claudewatch.db + config.yaml) -- NOT under /path/to/vault. MCP server registered in `~/.claude.json`. Installed via tools/install-claudewatch.ps1 then tools/wire-claudewatch-vault.py.

**Inputs.** Reads `~/.claude/projects` session transcripts (DefaultClaudeHome ~/.claude). CLI subcommands plus `claudewatch mcp` (stdio MCP server). config.yaml: scan_paths (default ~/code; /path/to/vault added best-effort), friction RecurringThreshold 0.30.
...
def t7_docs_present():
    ...
    if not (cm.exists() and "## Observability" in cm.read_text(encoding="utf-8", errors="ignore")):
        problems.append("AGENTS.md missing ## Observability")
...
def do_writes(txn):
    ...
    # D8a root-contract Observability section
    txn.edit(CONTRACT_MD,
             lambda c: safe_insert_before(c, CLAUDEMD_ANCHOR, CLAUDEMD_SECTION, "## Observability"),
             "D8a AGENTS.md Observability section")
    ...
    # D8c regenerate the derived .claude copy from canon (capture derived first for rollback)
    txn.capture_external(SKILL_MIRROR)
    if txn.dry:
        print("  would run:   python .agents/scripts/sync.py  (regenerate derived telemetry SKILL.md)")
    else:
        r = subprocess.run([sys.executable, ".agents/scripts/sync.py"], cwd=str(VAULT),
                           capture_output=True, text=True, timeout=120)
        if r.returncode != 0:
            raise WireError("sync.py failed: " + (r.stderr or r.stdout)[-200:])
...
# ---------------------------------------------------------------------------
VAULT_COMMIT_PATHS = [
    "wiki/hot.md", "tools/test-claudewatch.py", "AGENTS.md",
