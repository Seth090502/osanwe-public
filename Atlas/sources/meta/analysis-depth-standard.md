...
4. Exact portfolio impact
   - If held, read the live broker positions for every account in scope.
...
- Minimum 3 fresh SearXNG searches for any analytical response.
...
- If freshness is uncertain, search again before concluding.
- If SearXNG is down or materially degraded, label the output `LIMITED DATA` and avoid pretending current coverage is complete.
...
- Holdings source of truth:
  - the live broker read for every account in scope (never a local file)
- If held, calculate portfolio impact from actual share counts or token amounts.
- If not held, say "Not currently held."
- Do not fabricate cost basis or P&L numbers.
- Respect these standing rules:
  - Never report P&L for a position whose cost basis the broker cannot establish.
  - A share split changes share count and price mechanically while value does not.
  - Some cyclicals have a documented beat-then-selloff tendency after earnings; frame with that pattern when relevant.
...
Examples:
- better than adding to a broad index fund right now?
- worse risk/reward than an existing position?
- incremental improvement versus the last AMD view?
