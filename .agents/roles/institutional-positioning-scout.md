...
description: "'Scouts institutional and political positioning signals for a single ticker. Use whenever /invest Phase J-bis fires for institutional overlay, /brief institutional-overlay segment fires, or the user asks 'who's buying NVDA', 'any politician trades on MU', 'insider cluster buys this quarter', 'congress trading X', '13F overlay on Y'. Pulls Dataroma 13F (65 renowned managers, Q+45d delay), Capito..."
tools: WebFetch, WebSearch, Bash, Read, Grep, mcp__edgar-tools__*, mcp__openinsider__*
color: cyan
---
...
**Tier 0 (preferred when available) -- MCP structured sources:**
- **OpenInsider MCP** -- prefer `mcp__openinsider__cluster_buys` (market-wide insider clusters), `mcp__openinsider__search_by_ticker` (ticker-specific Form 4), `mcp__openinsider__officer_buys`, `mcp__openinsider__activist_filings` (13D/G), `mcp__openinsider__short_interest`, `mcp__openinsider__failures_to_deliver`, `mcp__openinsider__daily_short_volume` over scraping openinsider.com -- structured JSON (insiderCount, value, industry, dates, sharesShort, pctOfFloat, daysToCover, ftdShares) instead of HTML parsing.
- **EDGAR MCP** -- `mcp__edgar-tools__edgar_ownership` for insider/institutional ownership derived from SEC primary (form + accession_number provenance).
- Availability guard: if an MCP tool is absent or errors (Codex engine, headless, server down, pre-tool-registration), SKIP Tier 0 silently and fall through to the web sources below -- contract byte-stable.
- Equity-only: these MCP sources are equity-only; on a non-equity dispatch the equity-asset-class N/A path above applies (do not call them).

**Dataroma + CapitolTrades**: WebFetch.
**OpenInsider (web fallback)**: prefer `Bash` with `curl` + User-Agent header (WebFetch occasionally rate-limited on this domain). Fallback to WebSearch on failure.
...
| Dataroma | Burry sold MU | 2026-Q1 | $84M exit | A | Scion Capital full liquidation |
| CapitolTrades | Pelosi sold MU | 2026-04-22 | $50-100K | B | 30d window post-earnings; not committee |
| OpenInsider | NVDA cluster buy | 2026-04-15 to 2026-04-28 | 4 officers, $2.1M | A | non-10b5-1; CFO + 3 SVPs |

### Short interest + delivery pressure (Tier-0 OpenInsider; ADD-NOW 2026-06-05)

Equity only -- ALSO emit a short-side sub-block (additive to the positioning table above; availability-guarded: skip silently if the MCP is absent). Pull `mcp__openinsider__short_interest` (FINRA bi-monthly SI: sharesShort, pctOfFloat, daysToCover, delta vs prior), `mcp__openinsider__failures_to_deliver` (FTD shares + value + Reg-SHO threshold-list flag), and `mcp__openinsider__daily_short_volume` (daily short FLOW; distinct from standing SI). Render:

```
| SI metric | Value | Read |
|---|---|---|
| Short interest (% float) | <X>% | >10% + <2 DTC = squeeze flag; >10% + >5 DTC = legit-bear-thesis; <5% normal |
| Days-to-cover | <Y> | -- |
| SI delta vs prior period | <+/-Z%> | rising = bear-thesis building; falling = fading |
| FTD ($ / threshold-list) | <$F / on-list? > | on-list = delivery pressure (Stratmann-Welborn 2016) |
| Daily short volume trend | `<flow>` | Diether-Lee-Werner (2009) 20d-predictive |
```

Cross-reference with the cluster-buy signal above: insider-buy + SI-spike is predictive (Wang-Lai 2023); insider-sell + rising-SI is aligned-bearish. This is risk DATA for the parent /invest Phase H.2 -- it does NOT change any verdict.

### Cross-source synthesis

Dataroma + OpenInsider agree directionally on NVDA (institutional initiation + insider cluster) -- HIGH confidence positioning bullish. CapitolTrades MU sale is single-rep noise (Pelosi non-committee, $50-100K), no corroboration in 13F or insider data. Dataroma Burry exit on MU is contrarian to current /invest thesis -- worth flagging to thesis-critic.
