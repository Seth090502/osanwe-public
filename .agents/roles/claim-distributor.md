...
```json
{
  ...
  "claims": [
    ...
    {
      "fact": "thesis-theme-alpha fit CONFIRMED post-earnings",
      "source": "NVDA-analysis-2026-05-02",
      "source_date": "2026-05-02",
      "source_grade": "HIGH",
...
### Field map (parent payload -> distributor schema) [F028]

Parents send heterogeneous field names; normalize to the input schema above on receipt. `/ingest` Phase D / Phase E.0 is the reference payload:

| Distributor field | /ingest payload field | Value scale |
|---|---|---|
| `fact` | `claim_text` | free text (short declarative) |
| `source` | source-note stem | `[[stem]]` |
| `source_date` | claim / source `updated:` date | ISO `YYYY-MM-DD` |
| `source_grade` | `claim_grade` | HIGH \| MEDIUM \| LOW |
| `category_hint` | `section_target` | canonical section name / hint |
| `prov` | `prov` (pass-through) | `mcp:*` > `script:*` > `web:*` |

GRADE-SCALE MAPPING (the single rule -- no parallel vocabularies): `source_grade` carries exactly one of HIGH | MEDIUM | LOW -- the identical token `/ingest` Phase D assigned to `claim_grade` -- and that same token is written VERBATIM into the entity-note provenance parenthetical `(<grade>, per [[<source>]])`. HIGH | MEDIUM | LOW IS the canonical entity-note evidence scale end to end; there is NO A/B/C/D re-scaling at this boundary (the legacy A|B|C|D input label is retired). Contradiction-tiering authority order is HIGH > MEDIUM > LOW.

TRANSCRIBER TIER (2026-08-17, compounding-integrity fix). A claim may carry an `extractor` field. `extractor: local:<model>@<digest>` means a LOCAL model transcribed the figure -- the lowest provenance tier in the vault. It is orthogonal to `source_grade`: a local transcription of a HIGH-grade filing is still a local transcription, and `relay-verify` proves fidelity to the source, never that the source was right or that the value was attached to the correct metric. Two binding rules:
- An incoming claim with `extractor: local:*` contradicting an existing claim that has NO local extractor is **Tier 2b -- flag, never Tier 1 auto-resolve**, even when its date is newer and its grade equal or higher. Newer + local does not outrank frontier-fetched.
- When a local-extracted claim is inserted, carry the marker into the entity-note provenance parenthetical (`(<grade>, local-extracted, per [[<source>]])`) so the next run's drift detection can see what it is reading.
Without this the tier is dropped at the analysis boundary and a locally transcribed figure becomes an ordinary vault prior, then a J.5 drift signal, then a rating input -- laundered by nothing more than the passage of one run.
...
| `## Position` | Current shares + cost-basis snapshot, last decision date, doctrine-ceiling proximity |
| `## Sources` | Source list: `[[source-stem]]` / `[[<ticker>-analysis-<date>]]` wikilinks that contributed claims (set-union, maintained by parent -- NEVER a claim-routing target) |

If the entity note is missing a canonical section, mark the section as `op: create_section` -- parent inserts the heading + claim. **Placement is canonical-order-strict (X36 fix 2026-07-04):** set `anchor_after_section` to the NEAREST EXISTING section that PRECEDES the new one in the taxonomy table order above (Financial signals -> Thesis Fit -> Risks -> Catalysts -> Recent -> Position -> Sources); if no canonical section exists yet, anchor after the note's first H1 block. Never append at end-of-file by default and never place a section ahead of taxonomy order.

If a claim does not map to any canonical section (e.g., "CFO turnover at ticker" -- is that Risks, Catalysts, or Recent?): emit `op: flag` with `tier: 2`, `reason: "claim does not fit canonical taxonomy -- human review before insert"`, and `recommendation` field naming the candidate sections. Do NOT invent new sections; the canonical taxonomy is load-bearing for /spark cross-domain detection and /vault MOC-coverage classifier. New section taxonomy proposals belong in AGENTS.md edits, not in claim-distribution runs.
...
```json
{
  ...
  "edits": [
    ...
    },
    {
      ...
      "section": "Thesis Fit",
      "tier": 2,
      "tier_reason": "existing 'thesis CHALLENGED <date>' vs new 'thesis CONFIRMED <date>' -- qualitative conflict, human review",
      "incoming_claim": "thesis fit CONFIRMED post-earnings",
      "existing_claim_line": 124,
      "recommendation": "review <TICKER>-challenge-<date>.md vs <TICKER>-analysis-<date>.md; reconcile or accept latest"
    },
    {
      "op": "create_section",
      "section": "Catalysts",
...
- Section headings exact match -- canonical taxonomy strict (Financial signals, Thesis Fit, Risks, Catalysts, Recent, Position, Sources)
