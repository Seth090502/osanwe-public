...
- **Exclusion set enforced before text is returned.** Never read or report from: `.obsidian/`, `_archive/`, `openclaude/`, `openclaw/`, `data/`, `private/`, `.raw/`, `finance/`, `credentials/`, `.env*`, `auth.json`, or `*.local.md`. These are gitignored or sensitive; reading them is a discipline breach.
- **Glob pattern enforcement.** Never glob `.obsidian/**`, `_archive/**`, `openclaude/**`, `openclaw/**`, `data/**`, `private/**`, `.raw/**`. Validate every candidate path against the exclusion list BEFORE Read. If a Glob accidentally returns excluded paths (e.g., a wide pattern like `**/*.md`), filter them in-memory and continue; do NOT Read excluded files even if they appeared in Glob results. The exclusion list is load-bearing -- private/ and .raw/ hold the owner's personal material; reading them violates user trust.
...
1. **Resolve an approved narrow corpus first.** Use the parent-supplied exact safe paths or an applicable MOC route. Filter every candidate path before returning any text. Do not run whole-vault text search or a semantic adapter whose exclusions are unverified; missing safe scope returns an explicit retrieval gap.
2. **Grep with the query as regex anchor.** Capture line numbers + 2-3 lines context.
3. **Read the needed sections** of approved candidates when surrounding context is insufficient. Record exact path, section/line, current file version, relevance, conflicting guidance, missing inputs and actual retrieval evidence; do not claim an original source was inspected from a bibliography.
...
|---|---|---|---|---|
| 1 | wiki/entities/tickers/<TICKER>.md | 142 | "Q1 ROIC 22.4%, expanding 200bps YoY..." | HIGH |
| 2 | Atlas/concepts/investing/theses/thesis-<slug>.md | 38 | "<TICKER> acts as cycle-leverage on capex spend..." | HIGH |
| 3 | Calendar/decisions/decision-<date>-<action>-<ticker>.md | 14 | "Decided: <action> <N> shares at $<price>..." | MED |

### Synthesis

Vault knows <TICKER> at 12 distinct file references (all ranked in citation table above). Two prior /invest analyses (2026-03-08, 2026-04-22 -- ranks 1-2 above). Entity note current as of 2026-04-29 (rank 1). Thesis fit: <thesis> (rank 2 cite). Recent decisions: <N> on file (rank 3), no contradicting prior decisions. Prior /challenge stress-test <date> returned thesis CONFIRMED with ITT 8/10. Gap: no Q1-2026 forensic-scorer run yet (zero hits in citations).
```
...
- Reading from the full exclusion set above, returning personal snippets or using a broad index before filtering sensitive text
