...
"Computes the Top-12 forensic scoring stack on a single ticker. Use whenever /invest Phase K-bis fires, the user asks 'is NVDA a Piotroski 9', 'Altman Z on MU', 'Beneish flagging accruals', 'Greenblatt magic-formula yield', 'forensic check on X', 'quality scorecard for Y'. Pulls latest 10-K + trailing 4 10-Q from sec.gov + stockanalysis.com (primary); rejects macrotrends, fintel, simplywall as primary. Computes Piotroski F-Score (0-9), Altman Z (>2.99 safe / 1.81-2.99 grey / <1.81 distress), Beneish M (>-1.78 manipulation flag), Greenblatt magic formula EBIT/EV + ROC, plus ROIC, FCF yield, PEG, SBC/revenue, gross margin trend, accruals ratio, capex/revenue ratio, net-debt/EBITDA. Returns scorecard with HIGH/MED/LOW confidence per metric (data-quality + restatement risk). Use proactively whenever evaluating an investment-grade decision on a ticker with available 10-K + 10-Q -- forensic scoring is the floor evidence base, not optional. Read-only -- never writes."

# forensic-scorer

Top-12 forensic-quantitative scorecard. Single-ticker. This work requires 10-K/10-Q line-item interpretation, GAAP-vs-adjusted reconciliation, restated-statement detection, and framework-fit verdict require judgment beyond formula execution. Inherit the authorized session model and effort.
...
0. **EDGAR MCP (Tier 0, preferred when available)** -- call `mcp__edgar-tools__edgar_company` with the ticker for direct XBRL-parsed financials (4 annual + recent quarterly periods: income statement / balance sheet / cash flow) plus `recent_filings` (form + accession_number + filed date); `mcp__edgar-tools__edgar_ownership` for insider/institutional ownership. Provenance: every figure traces to a CIK + SEC accession_number -- the canonical immutable filing key, deterministically resolvable to the sec.gov filing -- which grounds evidence-grading and survives compaction. Availability guard: if the MCP tool is absent or errors (Codex engine, headless, server down, or pre-tool-registration), SKIP Tier 0 silently and fall through to Tier 1 -- output contract byte-stable. Equity-only: on a non-equity dispatch the equity-asset-class N/A path above applies (do not call it).
0a. **Filing-integrity red-flag (Tier 0, EDGAR; ADD-NOW 2026-06-05)** -- for the Restatement check, ALSO query `mcp__edgar-tools__edgar_search` for recent NT-10-K / NT-10-Q late-filing notices (Bartov-DeFond-Konchitchki 2017: accounting-reason NT filings carry significant negative CAR) and `mcp__edgar-tools__edgar_filing` (form 8-K) for Item 4.02 (non-reliance / restatement) and Item 4.01 (auditor change). Any NT-accounting-reason / 4.02 / 4.01 within 24mo = filing-integrity FLAG -> cap composite confidence at MED and surface it in the Restatement check section. Availability-guarded: skip silently on MCP absence; output contract byte-stable. This is forensic DATA -- it does NOT change the 12 metric formulas or the framework-verdict logic.
1. **SEC primary (web)** -- `sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=<ticker>` for 10-K + 10-Q. Authoritative.
...
|---|---|---|---|---|
| 1 | Piotroski F-Score | 9 binary tests on profitability/leverage/efficiency | >=8 strong; 3-7 mid; <=2 weak | PASS/MID/FAIL |
| 2 | Altman Z-Score | 1.2A + 1.4B + 3.3C + 0.6D + 1.0E | >2.99 safe; 1.81-2.99 grey; <1.81 distress | SAFE/GREY/DISTRESS |
| 3 | Beneish M-Score | 8-variable | <-2.22 clean; >-1.78 manipulation flag | CLEAN/FLAG |
...
| 11 | Accruals ratio | (NI - CFO) / total assets | <10% pref | CLEAN/FLAG |
...
| # | Metric | Value | Threshold | Verdict | Conf | Source |
|---|---|---|---|---|---|---|
| 1 | Piotroski F | 8/9 | >=8 strong | PASS | HIGH | 10-K FY2025 |
| 2 | Altman Z | 4.21 | >2.99 safe | SAFE | HIGH | 10-Q Q1-26 |
| 3 | Beneish M | -2.41 | <-2.22 clean | CLEAN | MED | TTM derived |
...
| 11 | Accruals | 2.1% | <10% | CLEAN | HIGH | 10-K |
...
Last 24mo restatements: none. Filing-integrity (EDGAR): no NT-10-K/10-Q late filings, no 8-K Item 4.02 (non-reliance) or 4.01 (auditor change). Beneish M and Accruals ratio both clean.
