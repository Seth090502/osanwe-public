...
### Mode routing (Pattern 6 deterministic)
...
Routing is deterministic (Pattern 6): the flag decides the mode, the bare argument decides the source channel (attachment / inline paste / filesystem path / vault path). `--preview` suppresses every write; `--refresh` and `--backlink-only` both require an existing vault path.

## Execution Rules

- **Phase N and Step N name the same stage.** Legacy "Step" numbering is preserved verbatim inside dispatch-report strings, gate literals, and report labels so downstream parsing and sessions-log greps keep working.
- **Subagent dispatch is MANDATORY when the harness exposes an Agent tool and the phase specifies it** (Phase 4b dispatches `frontmatter-linter` pre-flight before atomic commit). Pre-emptive skip based on judgment ("frontmatter looks fine", "I'll catch errors via ruamel gate later") is FORBIDDEN. The subagent (opus-pinned by its own definition) wraps `tools/frontmatter-check.py` and returns structured pass/fail JSON with specific defect list per the canonical frontmatter schema. Pre-flight catch is cheaper than mid-Edit failure cascade. Legitimate fallback fires ONLY on (a) contract violation -- subagent return missing required JSON fields, OR (b) actual dispatch failure -- timeout, rate limit, tool-denial, hard subagent crash. Pre-emptive skip on a harness that DOES expose an Agent tool surfaces in Step 6 Report as **DEVIATION** (not "fallback"). Quality preserved by additive design: existing Step 5a post-write ruamel gate remains intact; pre-flight is purely additive.
- **Harness without an Agent tool:** skip the dispatch and execute the documented inline fallback in Phase 4b verbatim -- run `python tools/frontmatter-check.py` over the composed frontmatter directly and read the same pass/fail + defect list off its output, applying the identical GATE / HARD DRIFT / SOFT DRIFT branch logic. That is a harness limitation, reported as `Step 4b frontmatter-linter: INLINE (no Agent tool)` -- never a DEVIATION.

### Quality Rules

- Body byte-exact is the contract: `sha256(target_body_bytes) == source_body_sha256` on every write path, `--replace` included. Divergence HALTs. /enrich never edits the document it onboards.
- Routing is DETERMINISTIC, never probabilistic: the first rule whose REQUIRED markers ALL match wins, and the report names the fired rule, its matched markers, and every rule checked before it (3c format). No confidence scores, no best-guess placement.
- Path-guards are mechanical and autonomous mode cannot override them: never write `.raw/`, `private/`, `_quarantine/`, `finance/`, `credentials/`, `.git/`, `.claude/hooks/`, `.claude/state/`. Autonomous placement landing there is a bug -- HALT.
- Confirmation is reserved for exactly two rules -- Rule 17 (thesis essay; sacred conviction artifact) and Rule 21 (new MOC; structural) -- plus the pathological sub-0.8x collision ratio in 3e. Everything else runs autonomously; asking for confirmation elsewhere is a defect, not caution.
- Collisions resolve by the 3e size-ratio heuristic (identical sha -> NO-OP; ratio >= 1.0x -> autonomous replace with the before/after report; ratio < 0.8x -> CONFIRM), never a silent overwrite of a larger target.
- Tag vocabulary is asserted mechanically before the write (2a) and re-asserted on the written state (5a): only `topic/`, `ticker/`, `company/`, `thesis/` namespaces, all namespaced. `domain/*`, `type/*`, and bare tags HALT.
- Back-linking is SYMMETRIC and idempotent: every wikilink in the onboarded doc's final `related:` earns a reciprocal reference on an existing, backlinkable target; already-linked targets are silent no-ops; per-file before/after sha256 + single-contiguous-insertion gates protect bodies outside the insertion site; a single failed file logs and continues rather than halting the batch.
- The /ingest recommendation is binary (YES / NO) and always emitted on a fresh onboard -- never a hedge, never omitted. `--refresh` and `--backlink-only` skip it because there is no new content to evaluate.
- Writes land ONLY under `Atlas/`, `Calendar/`, `Efforts/`, and `wiki/` per the placement table. `--preview` writes nothing and sets no F11 flag.
- One atomic commit per invocation (onboarded doc + symmetric back-links + type-specific additions): F11 set in Phase C before the primary Write, cleared in Phase K after gates pass; F14 narrow staging verified against the expected path set; F17 Co-Authored-By verified absent.
- ASCII only in every byte /enrich AUTHORS -- composed frontmatter, reports, commit messages (Pattern 22): `--`, `->`, `>=`, `<=`, straight quotes. The source body is the sole exemption; it is preserved byte-exact, non-ASCII included.
...
### Phase 1.a. target_path detection (v9 addition)
...
The `target_path` field is a user custom field per the AGENTS.md frontmatter schema (preserved byte-exact class alongside `ticker`, `sector`, `thesis`, `confidence`). Emitted by /deep v2-composed Research prompts so Research output can signal its intended placement to /enrich without slug drift.
...
- **Thesis clusters:** >=2 keyword matches -> `thesis/<stem>`:
    - theme-alpha: AI, data center, hyperscaler, capex, inference, accelerator
    - theme-beta: <keyword set withheld>
    - theme-gamma: grid, EV, battery, utilities, power capacity
    - theme-delta: platform, moat, network effect, ecosystem lock-in
    - theme-epsilon: broad market, index, core allocation
- **Topics:** reason over content, emit 3-5 lowercase-kebab primary topics -> `topic/<slug>`

### Phase 2a. Tag vocabulary guardrail (MECHANICAL)
...
### Phase 3. Determine placement (DETERMINISTIC, rule-based)
...
- IDENTIFYING: multiple `| col | col |` tables (>=3), explicit domain topic (geopolitics / ai-infrastructure / biomarkers / etc.)
- TARGET: `Atlas/sources/<domain>/ref-<slug>.md`
- Domain from primary topic/* tag (geopolitics+defense+macro -> investing; local-llm/quantization -> tech; methodology/meta -> meta). Slug from primary topic kebab-case.
...
- TARGET: `wiki/investing/snapshots/networth-<date>.md` (aligned to the /networth stem 2026-07-09; a `snapshot-` stem falls outside /networth Phase 2.5's historical-comparison glob)
...
- REQUIRED: multi-domain pattern-emergence framing AND explicit cross-references between tracked domains
...
**Rules 13-16 -- personal-domain routes.** Four routes for personal notes, not included in this copy.
...
The AGENTS.md reader/writer stance requires Atlas/ write confirmation. For /enrich autonomous mode:
...
### Phase 4. Compose canonical frontmatter
...
### Phase 4a. Related wikilinks -- existence annotation
...
### Phase 4b. DELEGATED dispatch to `frontmatter-linter` subagent (PREFERRED pre-flight path; Phase C wiring 2026-05-02)

Harness-conditional dispatch: when the harness exposes an Agent tool, dispatch `frontmatter-linter` (opus-pinned by its own definition); when it does NOT, execute the documented inline fallback below as the normal path. The pre-flight schema check itself is MANDATORY either way -- only the mechanism varies. It catches GATE breaches BEFORE the atomic-commit cascade in Step 5; mid-Edit failure is more expensive than pre-flight rejection.

**Inline fallback (verbatim; the normal path on a harness with no Agent tool):** run `python tools/frontmatter-check.py` over the composed frontmatter directly, read its pass/fail plus per-defect `field` / `issue` / `severity` / `fix_hint` output, and apply the SAME branch logic as the dispatch return (GATE -> HALT; HARD DRIFT only -> WARN and proceed; pass -> proceed to Step 5). Report it as `Step 4b frontmatter-linter: INLINE (no Agent tool)` -- a harness limitation, never a DEVIATION. Skipping the pre-flight where an Agent tool DOES exist remains a DEVIATION.
...
### Phase 5. F11 flag + primary Write (SET IN PHASE C, NOT PHASE E -- v7 FIX)
...
- **Subagent dispatch report** (Phase C wiring 2026-05-02): emit one of:
  ...
  - `Step 4b frontmatter-linter: FALLBACK (<reason>)` -- legitimate dispatch failure (contract violation or hard dispatch failure); ran the inline `tools/frontmatter-check.py` path and relied on the Step 5a post-write ruamel gate
  - `Step 4b frontmatter-linter: INLINE (no Agent tool)` -- harness exposes no Agent tool; ran the documented inline fallback as the normal path (NOT a DEVIATION)
  - `Step 4b frontmatter-linter: DEVIATION (<judgment>)` -- pre-emptive skip; flag as discipline breach + sessions-log entry

### Phase 6b. /ingest recommendation (v8 deterministic logic)
...
| Rules 13-16 (personal routes) | NO | Not included in this copy |
...
### Phase 7. Back-link application (SYMMETRIC, autonomous; --no-backlink skips, --confirm gates)
...
| Portfolio snapshot | `wiki/investing/snapshots/networth-<date>.md` | [wiki] | snapshot |
...
/ingest recommendation (v8):
  ...
  - Verdict: YES
  - Recommended next step:
      /ingest Atlas/sources/investing/ref-theme-alpha-deep-dive.md
...
The owner downloads a claude.ai Research mode output (generated from a /deep v2 prompt) and attaches + types /enrich. Source frontmatter includes:
    ...
    categories: [sources]
    type: reference
    target_path: Atlas/sources/investing/ref-theme-alpha-deep-dive.md
    created: 2026-04-22
    updated: 2026-04-22
    status: active
    confidence: high
...
v9 reads `target_path` in Phase 1.a; validates path-guards + extension + canonical-top-level; sets `explicit_target`. Phase 3 uses `explicit_target` directly (skips Rule 1-22). Placement: `Atlas/sources/investing/ref-theme-alpha-deep-dive.md` (exact /deep intent; no slug drift). Symmetric back-links fire on `related:` peers; knowledge-moc row added (type: reference); /ingest YES recommendation surfaces. Atomic commit. ONE user interaction; /deep-intended filename preserved end-to-end.
