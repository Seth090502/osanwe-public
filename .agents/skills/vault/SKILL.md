---
name: vault
description: "Vault integrity audit and maintenance. Use when asked if the vault is clean, to audit before a commit, or to check broken wikilinks, missing frontmatter, orphans, and stale refs -- and for the vault-health score. 9 classifiers via tools/vault-audit.py; 5 modes: audit, stats, daily, repair, refresh; 95/100 floor; never deletes. Not backup/restic freshness; not a lookup of note or template CONTENT."
metadata:
  categories: monitoring
  osanwe-risk: "critical"
  osanwe-effort: "max"
  osanwe-arguments: "mode"
  osanwe-argument-hint: "audit | stats | daily [--append `<text>`] | repair --apply `<ids>` | refresh [--scope `<domain>`]"
  osanwe-allowed-tools: "Read Write Edit Bash Glob Grep Agent"
...
- NEVER delete files. Move-to-archive only (`.claude/skills/_archive/` per `.agents/skills/AGENTS.md`).
...
- **The audit sweep is MANDATORY when specified by phase** (audit MODE Phase D). Harness-conditional dispatch: when the harness exposes an Agent tool, dispatch `vault-classifier-sweep` (it wraps `tools/vault-audit.py --json` and returns a structured findings JSON; the parent skill dispatches and validates the return); when the harness exposes NO Agent tool, execute the documented inline fallback (`python tools/vault-audit.py --json` directly) as the normal path, reported as `mode: inline (no Agent tool)` -- not a DEVIATION. Only the mechanism varies; the sweep itself never varies. On an Agent-tool harness, pre-emptive skip based on judgment ("script will probably work fine inline", "JSON parsing not worth the dispatch overhead") is FORBIDDEN and surfaces in the Phase L audit report as **DEVIATION** (not "fallback"). Legitimate fallback fires ONLY on (a) contract violation -- subagent return missing required JSON keys, OR (b) actual dispatch failure -- timeout, rate limit, tool-denial, hard subagent crash. Quality preserved by additive design: the direct script invocation remains as fallback for legitimate dispatch failures.
...
Not for: cross-domain pattern recognition (use /spark), session-end retrospective (use /retro), morning briefing (use /brief), document onboarding (use /enrich), claim extraction (use /ingest), task-list rollup (the /tasks skill is archived -- no live replacement), off-machine backup freshness (restic/B2 status is out of scope -- see the backup runbook, not this skill).
...
| `/vault refresh --scope <domain>` | Narrow refresh to one domain (`investing` / `meta`) |
...
2. Build `EXPECTED_ARCHIVED` set: DERIVE it from the live `.claude/skills/_archive/` folder listing (never a hardcoded pair); currently includes /research, /review (2026-04-26) plus /corpus, /telemetry, /tasks and withheld personal skills (2026-07-06 cut), plus prior version-bumped tombstones.
...
4. Build `CANONICAL_DAILY_SCHEMA`: 9 sections per `_templates/daily.md` (schema owner) + AGENTS.md: Market Pulse / Observations / Decisions / Commitments / Insights / Sessions Run / Cross-References / Tasks / Log.
...
6. Build `FORBIDDEN_FRONTMATTER_FIELDS`: `domain` (stripped per AGENTS.md schema), legacy fields.
7. Build `FORBIDDEN_TAG_NAMESPACES`: `domain/*`, `type/*`.

### Phase D -- Classifiers 1-5 (harness-conditional dispatch + script fallback)

Classifiers 1-5 come from `tools/vault-audit.py`. D.0 is the dispatched path (Agent-tool harnesses); D.1 is the direct-script path -- the normal path on a harness with no Agent tool, and the fallback everywhere else. Exactly one of the two runs.

#### Phase D.0 -- DELEGATED dispatch to `vault-classifier-sweep` (PREFERRED path where an Agent tool exists; Phase C wiring 2026-05-02)

**MANDATORY** (per Quality Rules) on any harness exposing an Agent tool: dispatch first, no pre-emptive skip. The subagent (Opus max for compute discipline) wraps `tools/vault-audit.py --json` and returns structured findings JSON keyed by classifier with severity tier per finding plus composite score.
...
On contract violation OR dispatch failure: fall through to the Phase D.1 inline script invocation below. Surface the fallback in the Phase L audit report.
...
#### Phase D.1 -- Direct script invocation (INLINE FALLBACK; the NORMAL path on a harness with no Agent tool, otherwise runs ONLY if Phase D.0 failed)

Execute `python tools/vault-audit.py --json` (the `--json` script computes 5 of the 9 canonical classifiers -- broken-wikilinks, missing-frontmatter, frontmatter-schema, orphan-detection, stale-refs -- plus the X1-X8 extension classifiers and the composite score; deprecated-skill-references, daily-continuity, template-drift, and MOC-coverage are NOT in the script and are populated inline by Phases E-I):
...
3. **Frontmatter-schema (forbidden fields)** (GATE) -- forbidden `domain:` field + `domain/*` / `type/*` tag namespaces (the script's `detect_forbidden_frontmatter`; the fuller categories-plural / status-enum check runs inline at Phase E)
4. **Orphan detection** (WARNING) -- entity files with <2 inbound wikilinks; exclude templates + archive folders
5. **Stale refs** (WARNING) -- `Atlas/sources/*/ref-*.md` with `updated:` >30 days; thesis-weighted annotation

Plus score-affecting HARD extensions X1 (session-artifact-gaps) + X2 (skill-body-length), and advisory SOFT extensions X3/X5 (count-drift), X6 (analysis-quality), X7 (stale-commitments), X8 (provenance-coverage). MOC-coverage gaps appear only in the script's markdown report (outgoing-link counts), never in `--json`.

Capture script output; parse into structured findings list. Report the mechanism honestly in Phase L: `mode: dispatched` | `mode: inline (no Agent tool)` | `mode: inline fallback (<contract violation | dispatch failure>)` | `DEVIATION`.
...
Grep vault for active-skill-list deprecations. The detection set MUST be derived from the `.claude/skills/_archive/` listing, not hardcoded: currently `/research`, `/review` (2026-04-26) plus `/corpus`, `/telemetry`, `/tasks` and withheld personal skills (2026-07-06 cut). Exclude:
...
- `AGENTS.md` historical sections

Pattern: `\b/(research|review|corpus|telemetry|tasks)\b` plus skill-invocation form `(use /research)` etc. -- build the alternation from the `.claude/skills/_archive/` listing rather than hardcoding this pair (this PROSE set is the /vault inline detection; the `tools/vault-audit.py` script-side classifier remains a /gate b BACKLOG item). Report any matches outside excluded paths. Severity: INFO (not breaking; cleanup candidate).
...
(Verified 2026-07-13: `_templates/daily.md` carries all 9 canonical sections -- no template drift. The earlier 7-section state was remediated.)
...
Byte-scan all NEW content. HALT on byte >127. Replace em-dash, smart quotes, ellipsis with ASCII. Record the Phase D sweep mechanism (dispatched / inline no-Agent-tool / inline fallback / DEVIATION) in the report.
...
1. file-counts-by-top-level-path (Atlas / Calendar / Efforts / wiki / private / docs / `.agents/skills/` canon active / `.claude/skills/_archive`)
...
15. thesis-essay-activity (mention count per `thesis/*`)
...
Skill counts are taken ONCE from the canonical tree (`.agents/skills/`) plus any skill whose canon is still a migration-window pointer stub (counted from `.claude/skills/`); the generated `.claude/skills/<name>/` mirrors of migrated skills are NEVER double-counted.
...
### Recommended `/goal` wrapper (B1 integration; Claude Code v2.1.139+ feature; skip where the harness has no equivalent)
...
The repair mode below remains single-pass-atomic per invocation. /goal sits ABOVE the skill, orchestrating multiple skill invocations until the structured evidence condition fires. On a harness with no `/goal` equivalent, the user re-invokes `/vault audit` + `/vault repair --apply <ids>` manually until the same structured evidence condition holds -- the halt condition is unchanged.
...
### Phase B: Load the most recent audit report
- Read `MOST_RECENT_AUDIT_PATH`; parse issue IDs from the FOLLOWUPS:skills block + the classifier sections
...
- **BLOCKED:** any deletion (move-to-archive only via `.claude/skills/_archive/`)
...
### Phase G: F11 set
- `touch .claude/state/auto-commit-disabled` -- only AFTER user confirmation, and BEFORE any Write
...
### Phase A: Pre-flight
- Same as audit Phase A (args + flags, today's ISO date, `OUTPUT_PATH` resolution, `-HHMM` collision check, F11 collision check)

### Phase B: State-transition print
- Same as audit Phase B: emit planned reads + planned writes + collision-handled output path to stdout BEFORE F11 is set

### Phase C: F11 set + load all `Atlas/sources/*/ref-*.md` AND every skill ref doc

Skill ref docs: glob BOTH `.agents/skills/*/ref-*.md` (canon) and `.claude/skills/*/ref-*.md` (generated mirrors + still-unmigrated skills), then dedupe by `<skill>/<ref-filename>` keeping the canon copy -- so a migrated skill's ref doc is ranked ONCE.

### Phase D: Compute staleness
- Per ref doc: days elapsed since its `updated:` frontmatter date
...
- Weights from `ref-stats-catalog.md`: theme-alpha=1.5, theme-beta=1.2, theme-gamma=1.0, theme-delta=1.0, theme-epsilon=0.8, meta=1.1 (personal domains carry their own weights and are not included in this copy)
...
- Write the refresh report + sidecar, F14 narrow stage, atomic commit, F17 verify, F11 clear -- then the audit-mode Phase P peripheral updates (sessions-log entry; hot.md bump), skipped under `--no-peripheral`

## Audit classifier reference

9 classifiers with detection rules + severity tiers + safe vs needs-confirm repair patterns in `ref-audit-classifiers.md` (~600-900 lines). The Phase D-H sections above list classifier names + severity; the ref doc holds full detection logic + repair-classification rules + worked examples per classifier.

## Statistics catalog reference

18 statistics with formulas + thesis-importance weights + Brier-score calibration mechanics in `ref-stats-catalog.md` (~400-600 lines). Stats mode Phase D-J lists statistic names; the ref doc holds computation logic + freshness-bucket boundaries.
...
- **/invest** -- /vault audit checks its output paths for orphans + frontmatter compliance

## Path-guards (mechanical)

NEVER write to: `.raw/`, `private/`, `finance/`, `credentials/`. NEVER delete (move-to-archive only via `.claude/skills/_archive/` per `.agents/skills/AGENTS.md`). All writes route through `wiki/maintenance/` (audit/stats/refresh/repair reports), `Calendar/decisions/sessions-log.md` (append), `Calendar/daily/` (today only, Sessions Run section append for `--append`), `wiki/hot.md` (last_audit/last_stats frontmatter bump), `_templates/daily.md` (template-drift repair only via /vault repair --apply T1).
...
| No Agent tool in the harness | D (audit) | non-dispatching harness | Run Phase D.1 `python tools/vault-audit.py --json` as the normal path; report `mode: inline (no Agent tool)` |
...
- `/tasks` -- archived skill (was a Tasks-plugin wrapper; not vault-integrity)
