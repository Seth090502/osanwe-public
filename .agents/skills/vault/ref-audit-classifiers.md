...
- **BLOCKED** -- never auto-applied; deletion is BLOCKED entirely (move-to-archive only via the archive directory)
...
1. Glob all `**/*.md` files (excluding `.git/`, `node_modules/`, `_archive/`, `.checkpoints/`, `.obsidian/`, `.smart-env/`, `<other-tool>/`)
...
Link: `[[challenge-example-thesis]]`
Detection: target `challenge-example-thesis` not in ALL_BASENAMES; closest match `challenge-example-thesis-2026-04-14`
Classification: SAFE-AUTO (single-candidate close match)
Proposed fix: replace `[[challenge-example-thesis]]` with `[[challenge-example-thesis-2026-04-14]]`
...
5. Check forbidden field `domain:` absent (stripped per the AGENTS.md schema)
...
Source: `wiki/research/example-note.md`
...
Detection: 1 inbound reference (only from `<private-area>/research/osanwe-public-readme-draft-2026-04-14.md`)
Classification: SAFE-AUTO
Proposed fix: add `[[<company>]]` to `Atlas/_MOCs/<private-file>.md` companies list
...
Stale ref docs prime stale analysis; the longer they go un-refreshed, the more likely subsequent /invest and /brief invocations build on outdated context.
...
Thesis weight: 1.0 (theme-epsilon)
...
1. Build `DEPRECATED_SKILLS` set from the `.claude/skills/_archive/` listing (authoritative; the set is derived from the archive listing), cross-checked against `Calendar/decisions/decision-log.md` deprecation entries
2. Build `EXCLUDE_PATHS` set: `.claude/skills/_archive/`, `docs/VAULT-HANDOFF-V*.md`, `Calendar/decisions/decision-log.md`, `Calendar/decisions/sessions-log.md`, `AGENTS.md` (historical context allowed)
3. Grep vault for skill-invocation patterns derived from the DEPRECATED_SKILLS set (`/<name>\b` + `(use /<name>)` per archived skill)
...
Source: `<private-area>/research/osanwe-public-readme-draft-2026-04-14.md` line 122
...
Source: `Atlas/_MOCs/<private-file>.md`
Detection: `wiki/entities/companies/<Company>.md` exists but not linked from `<private-file>` Companies section
Classification: SAFE-AUTO
Proposed fix: add `- [[<Company>]]` to `<private-file>` Companies list, alphabetically ordered
...
1. `STANDARD_EXCLUDES`: `.git/`, `node_modules/`, `_archive/`, `.checkpoints/`, `.obsidian/`, `.smart-env/`, `<other-tool>/`
2. `EXPECTED_ARCHIVED`: contents of `.claude/skills/_archive/` (derived from the listing at run time)
