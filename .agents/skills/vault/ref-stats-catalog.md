...
|---|---|---|
| theme-alpha | 1.5 | Highest-velocity refresh need; weight set per deployment |
| theme-beta | 1.2 | Thesis-defining references; refresh on catalysts |
| theme-gamma | 1.0 | Baseline reference weight |
| theme-delta | 1.0 | Baseline reference weight |
| theme-epsilon | 0.8 | Broad-index references age slowly |
| domain-a | 1.3 | High velocity for ref-doc refresh |
| domain-b | 0.7 | Low velocity |
| domain-c | 0.5 | Low velocity; a slow-moving evidence base |
...
- Exclude: `.git/`, `node_modules/`, `.checkpoints/`, `.obsidian/`, `.smart-env/`, `<other-tool>/`
...
### Formula
```
held_tickers = parse(broker holdings read) -> ticker symbols
...
### Edge cases
- Combined-account de-dup: one ticker present in more than one account counts as 1 ticker
- ETFs and equities both included (a fund symbol counts as a ticker)
- Assets tracked outside the equity accounts (if any): separate sub-stat
...
### Formula
```
for each domain D in [investing, meta]:
...
## Statistic 6: (reserved)

A personal-domain statistic, not included in this copy.
...
For each `thesis/*` (5 theses):
```
mention_count_30d = grep -r "thesis/<slug>" across vault (last 30d files only)
