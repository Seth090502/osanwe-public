# /synthesis spark mode -- reference

- **Harness-conditional dispatch.** Dispatch `pattern-class-scout` (per-class fan-out) when an Agent (subagent) tool exists in the running harness; when it does NOT, execute the documented inline Phase E.1 detection for every class verbatim as the NORMAL path, reported as `mode: inline (no Agent tool)` -- a harness capability gap, never a DEVIATION. Skipping the dispatch where an Agent tool DOES exist remains a DEVIATION. The 9-class pattern detection itself is MANDATORY either way -- only the mechanism varies. No phase conditions its behavior on model identity.

- **Tier-A workflow dispatch satisfies the MANDATORY-dispatch rule** (vNEXT): on TOPOLOGY=dw, invoking the `spark-sweep` workflow (which dispatches the same `pattern-class-scout` instances via agentType) IS the mandatory dispatch; DEVIATION semantics apply identically in both topologies (pre-emptive skip of a class without legitimate failure = DEVIATION, surfaced in Phase N). The per-class convergence gate (contract violation/dispatch failure -> one re-dispatch -> inline Phase E.1 fallback for that class only) mirrors the Agent-tool path exactly. The Tier-B sequential spine is the PERMANENT UNIVERSAL FALLBACK: any harness without the Workflow tool -- and any harness without an Agent tool at all -- runs it. See `ref-dw-topology.md` (sibling of this file).

### Quality Rules (quality standards preserved verbatim from v1)

- This is NOT a summary skill. Do not summarize what happened. Find what CONNECTS.
- The value is in surprising connections, not obvious ones. "NVDA and AMD are both semiconductors" is worthless. "A training-load pattern in one domain mapping to a cadence in another" is a spark.
...
| Syntax | Behavior |
|---|---|
| `/spark` or `/spark scan` | Wide cross-domain scan, auto-widened default timeframe (formula below the table), top 7 sparks |
| `/spark scan 7d` / `30d` | Explicit timeframe override |
| `/spark focus <target>` | Narrow scan to one domain (`investing`/`meta`), one tag (`thesis/<slug>`), or one entity stem (`<TICKER>`); cap 4 sparks |
...
Default with no arguments: `scan` mode. Default timeframe (vNEXT2 auto-widen): `min(30, max(14, days_since_last_spark + 3))` days, where `days_since_last_spark` derives from hot.md frontmatter `last_spark` (missing field -> 14). Deterministic; an explicit `7d|14d|30d` argument always overrides. Rationale: a dormant pattern layer must not resume with a window that misses the dormancy itself.

## Process (Phase 0 vault retrieval + 14 atomic phases A-N incl. Phase H.5 verify wave + Phase O write + Phase O.0 audit gate + Phase P peripheral)
...
1b. Resolve the window (vNEXT2): no explicit timeframe argument -> read `wiki/hot.md` frontmatter `last_spark` NOW (single-field read; full hot.md load stays in Phase C) and compute `window_days = min(30, max(14, days_since_last_spark + 3))`; missing field -> 14. Explicit `7d|14d|30d` always overrides. The computed window feeds the Phase B print and every downstream phase.
...
6. Topology detection (deterministic; NEVER halts). TOPOLOGY = `dw` iff the Workflow tool is present in the orchestrator's current session tool surface; else `sequential` (Codex engine, headless, restricted sessions, any DW regression). Ambiguity -> `sequential` (safe fallback). `tools/lib/capability-detect.sh` (`OSANWE_DW_HINT`) is ADVISORY only -- the tool-surface check is authoritative. Forced-sequential knob (R2): `OSANWE_FORCE_SEQUENTIAL=1` (env, or explicit operator instruction) forces TOPOLOGY=sequential for test/regression runs; there is deliberately NO force-dw counterpart (dw without the Workflow tool breaks; forcing the fallback is always safe). Also record AGENT_TOOL = present|absent (does this harness expose an Agent/subagent tool?) -- `dw` implies present; `sequential` + AGENT_TOOL=absent routes Phase E to inline detection as its normal path. Record ORCHESTRATOR_MODEL = the active model string. PASSIVE LOGGING ONLY: no behavior ever branches on model identity; TOPOLOGY + AGENT_TOOL + ORCHESTRATOR_MODEL flow to the Phase B print and the meta.json sidecar (additive fields). Full Tier-A spec: `ref-dw-topology.md` (sibling of this file).
...
Emit to stdout the planned reads + planned writes + collision-handled output path. User can abort here pre-flag. Format:
    ...
    /spark `<mode>` -- planned state transition:
      reads (10 file groups): daily notes (timeframe), sessions-log, decision-log, hot.md,
        insight-stream, briefings `<timeframe>`, entity notes `<timeframe>`, prior 3 sparks,
        gates-registry + in-window gate sheets, execute-or-decline ledger
      writes:
        - `<OUTPUT_PATH>` (NEW)
        - `<report-stem>`.meta.json (NEW sidecar; .md REPLACED by .meta.json, never appended)
        ...
        - <=3 prior spark sidecars (prediction-resolution write-back; additive JSON, prediction_scoring keys only)
      ...
      Agent tool: <present (pattern-class-scout fan-out) | absent (inline-no-Agent-tool spine)>
...
1. Set F11: `touch .claude/state/auto-commit-disabled` BEFORE any Write (the marker the per-write auto-commit hook reads; harnesses without that hook honor the same lifecycle by instruction -- [[ref-execution-discipline]]).
2. Parallel Read 10 file groups (Pattern 1; groups 9-10 are vNEXT2):
   ...
   - `wiki/research/gates/gates-registry.md` + gate sheets dated within timeframe (vNEXT2 group 9: verdicts, mandates, outcome column, Compliance blocks -- FOMO-SUSPECT clusters, ROUTING-AROUND flags, and mandate follow-through gaps are behavioral/decision-divergence evidence)
   - `Calendar/decisions/execute-or-decline.md` (vNEXT2 group 10: full ledger -- overdue PENDING rows, decline-vs-execute shapes, escalation ages are failure-mode/negative-space evidence)
...
4. Corpus-extract resolution (vNEXT S4 + vNEXT2; orchestrator-resolved MAIN-LOOP, never by scouts): resolve the latest `wiki/maintenance/telemetry-*.md` path (+ an in-session claudewatch summary when its MCP tools are reachable; headless = degrade to the report file), the `wiki/investing/calibration-monitor.md` Call Log rows, the latest overnight ratification queue (`wiki/research/overnight-summary-*.md` RATIFICATION QUEUE section) + `wiki/maintenance/proposals/` inventory, and the master-context Part W honest-gaps TEXT (extracted via the live Part index offsets from the SessionStart injection; sensitive Parts B/N3/X never extracted). vNEXT2 extracts: `gates_extract` (gates-registry Sheet Index rows + Compliance blocks + in-window sheet verdict/mandate/outcome tuples), `eod_ledger` (execute-or-decline rows text), `kernel_journal_rows` (in-window `wiki/investing/analyses/*-analysis-*.md` kernel frontmatter tuples: rating / confidence / conviction / scoring_path / deployment_override -- the journal.base source data), `projects_index` (`wiki/projects/projects-index.md` rows). Pass ALL via `spark-sweep` workflow args (Tier-A) or inline prompt context (Tier-B) per the per-class wiring table in `ref-dw-topology.md` (sibling of this file) sec 8. Scouts NEVER read the master-context doc directly. R1 TEMPORAL FENCE: on retrospective runs (window_end < today) every extract -- including all four vNEXT2 extracts -- is filtered to artifacts dated <= window_end and marked `as_of`; spark-sweep.js drops unmarked retrospective extracts fail-closed.
...
Prediction-resolution WRITE-BACK (vNEXT2; closes the open-loops PREDICTION class): scoring a prior spark's prediction ONLY in the new run's sidecar leaves the PRIOR sidecar's `prediction_scoring.unresolved > 0` forever -- exactly what `tools/open-loops.py` scans (PREDICTION loop class, weight 2.5; it globs `wiki/research/sparks/*.meta.json`), so resolved predictions kept nagging. SIDECAR NAMING (red-team 2026-07-09): the on-disk convention is `spark-<date>.meta.json` -- the report stem with `.md` REPLACED by `.meta.json`, never appended -- and the write-back MUST glob the real sidecar (`<report-stem>.meta.json`); a `-meta.json` suffixed file is an orphan open-loops cannot see. Therefore, for EACH prior prediction scored resolved-true/resolved-false this run: update THAT prior spark's `<report-stem>.meta.json` in place -- inside its `prediction_scoring` object, append per-prediction `resolutions[]` entries `{test, status: resolved-true|resolved-false, resolved_by: "spark-<today's stem>", resolved_on: "<ISO date>", check: "<one-line deterministic check result>"}` and decrement its `unresolved` count accordingly (floor 0). ADDITIVE-ONLY JSON edit: no key outside `prediction_scoring` may change; unresolved predictions stay untouched. These sidecar writes stage with the atomic commit (Phase P) and appear in the Phase B state-transition print.
...
Two paths, one contract: E.0 delegated scout dispatch (preferred wherever the harness exposes an Agent tool, both topologies) with E.1 inline detection as the per-class legitimate-failure fallback AND as the normal path on a harness with no Agent tool; the scout return contract below binds both.

**Harness routing (deterministic, from Phase A item 6):** Agent tool present + TOPOLOGY=dw -> `spark-sweep` workflow fan-out. Agent tool present + TOPOLOGY=sequential -> direct `pattern-class-scout` dispatch (the Tier-B spine below, verbatim). Agent tool ABSENT -> the main loop runs Phase E.1 inline for all 9 classes (or the single theme class) against the same contract, reported as `mode: inline (no Agent tool)` in Phase N -- not a DEVIATION, not a "fallback".

#### Phase E.0 -- DELEGATED dispatch to `pattern-class-scout` (PREFERRED path where an Agent tool exists; Phase C wiring 2026-05-02)

**Topology routing (vNEXT):** on TOPOLOGY=dw (Phase A item 6), read `ref-dw-topology.md` (sibling of this file; read-on-demand companion) and invoke the `spark-sweep` workflow (`.claude/workflows/spark-sweep.js`) with args `{classes (1 for theme / 9 for scan), window_days, window_end, run_timestamp, mode, session_id, vault_context_summary, corpus_extracts (Phase C item 4 -- incl. vNEXT2 fields gates_extract, eod_ledger, kernel_journal_rows, projects_index), prior_sparks_summary, token_budget (SPARK_DW_TOKEN_BUDGET)}`. It dispatches the SAME `pattern-class-scout` instances via agentType (scout .md contract + opus max definition UNCHANGED; the workflow's JSON schema wraps the markdown contract 1:1) in chunked waves <= 6, applies the per-class convergence gate (`no_patterns: true` = SUCCESS; contract violation/dispatch failure -> one re-dispatch -> `_fallback` marker -> inline Phase E.1 for that class only), and returns the scout bundle. The dispatch text below is the TOPOLOGY=sequential (Tier-B) path, retained VERBATIM as the universal fallback -- and it remains the authoritative spec of what each scout return must contain in BOTH topologies.
...
Per dispatch, use the Agent tool with subagent_type `pattern-class-scout`. Pass input: `{class: "<one of 9 classes>", time_window: "<from Phase A item 1b (auto-widened)>", session_id: "<current-session-id>"}`. Expected return per class: continuity audit (vs prior 3 spark reports) + 0-5 ranked sparks with calibrated % confidence + evidence anchors (file + date + line) + downstream skill recommendation.
...
- **TOPOLOGY=sequential (the universal fallback, incl. every harness with no Agent tool):** an inline evidence spot-check on the top-2 promoted sparks replaces the wave -- the main loop opens every cited anchor itself; same four-lens refute semantics, same fail-closed default. Reported as `verify: inline (no Agent tool)` where no Agent tool exists; never a DEVIATION. `--verify` forces the inline spot-check on ALL promoted sparks.
- **Outcomes:** any refuted lens -> the spark drops to `patterns_below_threshold` with the refutation reason (transparent suppression, never silent); DEMOTED -> Continuity Audit table as PERSISTED with the prior_match named; survivors proceed with `recalibrated_confidence`. Phases I and J compute over SURVIVORS only. Record the tally `{fired, refuted, demoted, survived}` in meta.json `verify_wave` + the report footer (gate item 19). Full lens contracts: `ref-dw-topology.md` (sibling of this file) sec 5.
...
For each promoted spark, generate 0-2 concrete downstream candidates. Block bounded to <=5 entries total per report. Format:
    ...
    - /consolidate topic `<topic>` -- spark N PERSISTED across >= 2 consecutive audits; playbook-promotion candidate (trigger: EOW)
    <!-- /FOLLOWUPS:skills -->
...
### Phase K -- Pre-Output HALT gate (22-item invariant check; items 1-18 preserved verbatim from v2)

Before any Write, verify all 22:
...
21. Topology + agent_tool + orchestrator_model logged in meta.json sidecar (passive fields; never branched on)
22. Prior-sidecar prediction write-backs (when any): additive-only JSON confined to `prediction_scoring` (resolutions[] + unresolved decrement), each target sidecar re-parses as valid JSON, all staged in this commit; explicit `(no prediction resolutions this run)` otherwise
...
Dispatch report (one line, mandatory): `Phase E pattern-class-scout: DISPATCHED (<N> classes, <dw|sequential>)` | `INLINE (no Agent tool in this harness)` | `FALLBACK (<class-list>: <reason>)` | `DEVIATION (<judgment>)` -- DEVIATION is a discipline breach, logged to sessions-log. Same three-way vocabulary applies to the Phase H.5 verify wave.

### Phase O -- Atomic write

1. Write `OUTPUT_PATH` (spark report).
2. Write the sidecar at `<report-stem>.meta.json` (OUTPUT_PATH with `.md` replaced by `.meta.json` -- the open-loops glob convention; never append `-meta.json`).
...
4b. **Prior-sidecar prediction write-backs** (vNEXT2; when Phase D resolved any) -- apply the additive-only `prediction_scoring` updates to each scored prior `<report>-meta.json`; re-parse each as JSON post-write (parse failure -> F.halt, restore from git).

5. F14 narrow-stage all touched files (incl. 4b sidecars); atomic commit with prefix `spark: <mode> -- <N> patterns + continuity-<N>`.
...
NEVER write `.raw/`, `private/`, `finance/`, `credentials/` (Phase K item 15).
...
| Insufficient context | C | <timeframe of activity nearly empty | HALT with "no actionable patterns; <X> files in timeframe" -- valid output per Quality Rules |
| Pre-Output gate failure | K | any of 22 invariants violated | HALT with item-specific diagnostic; F11 stays on; fix and retry |
...
| No Agent tool in the harness | A / E.0 / H.5 | Non-dispatching harness (AGENT_TOOL=absent) | Run Phase E.1 inline for all classes + the Phase H.5 inline spot-check as the NORMAL path; report `mode: inline (no Agent tool)`; never a DEVIATION |
...
| Sidecar write-back parse failure | P.4b | prior meta.json invalid JSON post-edit | F.halt; `git checkout -- <sidecar>`; fix additive edit; retry (unresolved count must never be corrupted -- open-loops depends on it) |
...
### Example 2: focus on theme-alpha thesis

`/spark focus thesis/theme-alpha` -> narrowed reads (only files tagged `thesis/theme-alpha` or referencing held tickers in that thesis); detects across all 9 classes filtered to thesis scope; cap 4 sparks; writes `wiki/research/sparks/spark-thesis-theme-alpha-2026-04-26.md`. Useful before /challenge thesis-theme-alpha.
