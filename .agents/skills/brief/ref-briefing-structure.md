...
Quiet-day BLUF. On low-signal days the BLUF still exists; it states the absence of actionable signal and names the posture. Example class: "No thesis-invalidating signal overnight; theme-epsilon posture unchanged; focus on Thursday CPI." The anti-pattern is skipping BLUF on quiet days, which forces the reader to reconstruct posture from subsequent sections.
...
Portfolio Health is expressed as a single letter grade (A through F), not a narrative summary. The grade summarizes the joint status of all five active theses weighted by position. The rubric is deterministic: A if all five theses are HEALTHY and coherence is ALIGNED; B if at most one is WATCH; C if two or more are WATCH or one is STRESSED; D if two are STRESSED; F if any thesis is INVALIDATED.
...
The Signal Dashboard is an eight-column table tracking cross-asset signals relevant to the five active theses. Columns are: Signal, Level, Change, Range Position, Regime Fit, Thesis Affected, Grade, Composite. Inclusion requires that a signal materially influence at least one active thesis; inclusion on novelty alone is forbidden.
...
The Thesis Status Board is a three-column table: Thesis, Status (binary among HEALTHY, WATCH, STRESSED, INVALIDATED), Evidence (with inline citation). Five rows, one per active thesis. Status is deterministic, not narrative; the rule-set lives in `ref-monitoring-rules.md` and in each thesis file. The Evidence column is mandatory and carries the grade.
...
Peer-constellation signaling exploits the cross-sectional pattern whereby pre-announcements within a tight peer set forecast the target firm's surprise direction. The linkage to the five active theses is explicit: if NVDA's pre-announcement signals theme-alpha demand acceleration, the theme-alpha Thesis Status Board entry inherits that signal at the specified evidence grade. The Pre-Earnings Monitor is the mechanism by which that inheritance is traced.
...
<!-- FOLLOWUPS:skills
skill: update-thesis
  args: { thesis: "theme-alpha", reason: "NVDA pre-announcement"}
  trigger: "status transition HEALTHY -> WATCH"
skill: rebalance-draft
  args: { scope: "theme-alpha sleeve", constraint: "no increase above 32% sleeve"}
...
priced_in_calls:
  - event: "CPI 2026-04-24"
    ...
    ev_bps: -18
conviction: 0.62
bluf: "theme-alpha HEALTHY heading into a <ticker> print; position sensitivity asymmetric to downside."
counter: "CFTC positioning at 95th percentile; reversion would invalidate HEALTHY read."
alternative: "Peer semis miss while the leader beats; dispersion regime persists (15%)."
signal_dashboard_summary: "11 signals; composite positive for theme-alpha, mixed for theme-epsilon."
warnings_count: 2
gaps_count: 1
