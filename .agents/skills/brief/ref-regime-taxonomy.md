...
Peer-constellation signaling is an earnings-regime tactic: one company's results carry read-through for peers that report later. **SK Hynix reporting 2--4 weeks before Micron provides DRAM and HBM pricing, HBM bookings, and Nvidia supply-chain read-through for the memory group.** Q1 2026 SK Hynix results (revenue 52.58 trillion KRW, operating margin 72%, HBM sold out through 2026) were reported on [CNBC 2026-04-23](https://www.cnbc.com/2026/04/23/sk-hynix-earnings-ai-memory-shortage-hbm-demand.html) with matching [Korea Times coverage](https://www.koreatimes.co.kr/business/tech-science/20260423/sk-hynix-q1-operating-margin-breaks-above-70-higher-than-nvidia-tsmc); the memory-supercycle context is framed in [Introl's 2026 HBM analysis](https://introl.com/blog/ai-memory-supercycle-hbm-2026) projecting TAM from ~$35 billion in 2025 to ~$100 billion by 2028. Similarly, AMD results carry read-through to NVDA on data-center GPU demand; TSMC monthly revenue carries read-through to the full AI-infrastructure supply chain.
...
### Sensitivity matrix by asset bucket (illustrative)

Cells indicate expected impact direction (+/-/0) and magnitude bucket (S/M/L).

| Bucket | Crisis | Risk-off | Fed Week | Earnings | Rotation | Risk-on | Early | Mid | Late | Contraction | Coh-On | Coh-Off | Diverg |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| High-beta growth equity | -L | -M | 0 | +/-L | -M | +L | +M | +L | +/-M | -L | +L | -L | -M |
| Cyclical equity | -L | -M | 0 | +/-L | -M | +M | +L | +M | -M | -L | +M | -L | -M |
| Sector ETF | -L | -M | +/-M | +/-M | -M | +L | +M | +L | -M | -L | +L | -M | -S |
| Broad index ETF | -M | -M | +/-S | +/-S | 0 | +M | +M | +M | 0 | -M | +M | -M | -S |
| Large-cap crypto | -L | -L | +/-M | 0 | +/-M | +L | +M | +M | -M | -L | +L | -L | -L |

Readings: high-beta growth equity carries Large negative Crisis exposure because the long-duration, high-multiple, cyclical-demand profile compounds on the downside. A diversified index ETF is the lowest-beta bucket by construction; a large-cap crypto asset has the widest conditional return distribution because of its sensitivity to liquidity conditions. Fed Week impact is sign-indeterminate because it depends on dove-versus-hawk outcome -- the cells reflect magnitude, not direction. Earnings cells are sign-indeterminate reflecting the pre-print uncertainty that collapses within 48 hours of the report.
