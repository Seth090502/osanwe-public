...
|---|---|---|---|---|
| `DGS10` | 10Y Treasury yield | Markets line + /invest disclosure, the rate_shock nominal-confirm screen, and the smoothed Rf input (the 10Y LEVEL is NOT a deployment gate -- retired 2026-07-30) | Percent | daily |
...
| `DFII10` | 10Y real yield (TIPS) | **/invest deployment rate_shock gating series** (>=90 obs; 60-obs lookback + hysteresis reconstruction) + gold/crypto/duration context | Percent | daily |
| `WALCL` | Fed balance sheet (total assets) | net-liquidity composite input | $ Millions | weekly (Wed) |
| `RRPONTSYD` | ON RRP (overnight reverse repo) | net-liquidity composite input | $ Billions | daily |
| `WTREGEN` | Treasury General Account (TGA), week average | net-liquidity composite input | $ Millions | weekly (Wed) |
| `DTWEXBGS` | Broad dollar index | **Grade-A macro-regime dollar input** (replaces script DXY for regime work; script DXY stays in the Markets one-liner) | Index | daily |

Convert HY OAS percent to bps for the day-regime table (e.g. 2.75% = 275bps).

**Net-liquidity composite (brief-vnext 2026-07-04; units corrected 2026-07-15)**: `Net liquidity = WALCL - RRPONTSYD*1000 - WTREGEN` (all terms in $M before subtracting; **WALCL and WTREGEN are $M -- RRPONTSYD ALONE is $B and is the only term needing a x1000 scale**). The prior wording ("WALCL is $M, the other two $B") mislabelled WTREGEN as billions, which executed literally subtracts ~$774T from a ~$6.7T balance sheet; RRPONTSYD's billions label was always correct. Both re-verified against FRED series metadata 2026-07-15 [prov: mcp:fred:fred_get_series]. BASIS CAVEAT: WALCL is a Wednesday point-in-time level while WTREGEN is a WEEK AVERAGE, so the composite mixes bases -- `WDTGAL` (TGA Wednesday Level, $M) is the basis-correct partner and differed from WTREGEN by $24.8B on 2026-07-08 (749,244 vs 774,062). Swapping to WDTGAL is a behaviour change, not a label fix, and is left to /decide. RRPONTSYD is daily and prints ahead of the weekly series, so the composite is only coherent on Wednesday-aligned dates; ON RRP is now a dead term ($151M on 2026-07-15 vs a ~$2.5T peak), so variance is effectively WALCL - TGA. Trend reported as the 4-week delta. Rendered as ONE line under the Markets one-liner: `NetLiq $X.XXT [4wk +/-$XXB] | Broad$ XXX.X`. Reported CONTEXT only -- not a regime axis; a legal Warning Problems input when the 4wk delta < -$150B. Availability-guard applies (FRED absent -> skip the line silently).
...
| Series | Value | Read |
|---|---|---|
| DGS10 | 4.49% | context only; the 4.30/4.40 deploy thresholds were retired 2026-07-30 |
| DGS2 | 4.08% | -- |
