...
## Position
<parent-maintained position context; NOT a claim-routing target -- recognized for byte-range preservation only>
...
Every CREATE operation emits a back-link cascade into at least one Map-of-Content (MOC) note. Tickers cascade into `Atlas/_MOCs/investing-moc.md`; companies cascade into the companies map of content by default, with routing to a domain MOC (`aerospace-moc.md`, `semiconductors-moc.md`) when the entity's sector matches a registered domain-MOC entry in the `MOC_STEMS` index. The cascade is symmetric: the entity note's frontmatter `related:` field gains `[[<moc-stem>]]`, and the MOC body gains a bullet line under its appropriate grouping. This mirrors Wikipedia's category-membership discipline where every substantive article belongs to at least one category and every category page lists its members [17]. Symmetric maintenance of both directions is what makes Obsidian's graph-view traversal accurate; Obsidian resolves wikilinks by text match rather than by explicit ID, so the symmetric discipline is the only defense against orphan links [18].
...
The UPDATE branch begins by reading the existing note into memory and computing `before_sha256 = hashlib.sha256(body_bytes).hexdigest()` where `body_bytes` excludes the YAML frontmatter and the trailing newline-delimited sentinel that separates frontmatter from body [21]. The frontmatter is parsed with `ruamel.yaml` in round-trip mode so that key order, quoting style, and comment placement are preserved across the eventual re-serialization [15]. Section offsets are detected by scanning for the canonical H2 headers (`## Overview`, `## Financial signals`, `## Thesis Fit`, `## Risks`, `## Catalysts`, `## Recent`, `## Position`, `## Sources`) and recording byte ranges (`## Position` is parent-maintained and never a claim-routing target -- it is scanned so its byte range is preserved, per the W4a canonical order); under `Recent`, the nested H3 ISO-date subheaders are enumerated so that a new dated event can be inserted at the correct chronological position.
...
Confirmation (invest-enhancement-pass 2026-07-11): the metric families added by that pass -- deterministic-technicals panel figures, tax-lot/LT-conversion facts, FMP/proxy forward estimates, and options-flow/structure observations -- route through this SAME table with no schema change (technicals and forward estimates match priority 5, tax-lot conversion dates priority 2, options-flow positioning priority 3/4 by semantic); no new target section exists or is needed.
