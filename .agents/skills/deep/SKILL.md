---
name: deep
description: "Use when asked to build, compose, or prep an institutional-grade Deep Research prompt for claude.ai Research mode: a 100+ source brief on a market, sector, supply-chain, or thesis topic. Composes the PROMPT only; never runs the research. Emits target_path frontmatter for zero-friction /enrich onboarding. Not code deep-dives, held-position analysis (/invest, /challenge), or ad-hoc lookups (use WebFetch)."
metadata:
  categories: analysis
  osanwe-risk: "safe"
  osanwe-effort: "max"
  osanwe-arguments: "target"
  osanwe-argument-hint: "`<topic-slug>` | `<ticker>` | `<thesis-stem>` [--interactive] [--preview]"
  osanwe-allowed-tools: "Read Write Bash Grep Glob Agent"
...
- Code exploration ("deep dive on this codebase") -- unrelated sense of the word
...
Routing is deterministic (Pattern 6): the flag decides the mode; the bare argument decides the framing. `--preview` suppresses every write; `--interactive` inserts a clarifying-question turn before Phase 3.

## Execution Rules

- **Phase N and Step N name the same stage.** Legacy "Step" numbering is preserved verbatim inside dispatch-report strings and audit literals so downstream parsing and sessions-log greps keep working.
- **Subagent dispatch is MANDATORY when the harness exposes an Agent tool and the phase specifies it** (Phase 2.5 dispatches `source-aggregator`). Pre-emptive skip based on judgment ("I can find sources inline", "the topic is well-covered already") is FORBIDDEN. The subagent decides source-quality applicability via its own return contract; the parent skill dispatches and validates the return. Legitimate fallback fires ONLY on (a) contract violation -- subagent return missing required fields (table columns, abstracts, blocked-domain summary), OR (b) actual dispatch failure -- timeout, rate limit, tool-denial, hard subagent crash. Pre-emptive skip surfaces in Phase 6 audit as **DEVIATION** (not "fallback"). Quality preserved by additive design: existing Phase 2 vault-context Glob/Grep stays intact; source-aggregator adds external candidate sources on top.
- **Harness without an Agent tool**: skip dispatch and execute the documented inline fallback in Phase 2.5 verbatim. That is a harness limitation reported as `FALLBACK (no Agent tool in this harness)` -- never a DEVIATION.

## Process

### Phase 1. Load portfolio context (parallel Read)

Read wiki/hot.md and permitted thesis/entity references for context. Account quantities for the CONTEXT block require runtime broker read evidence; absent it state UNVERIFIED and omit quantities. Never read private/ or *.local.md into the prompt.

Extract: current positions with cost basis + account split; active thesis exposure percentages (flag doctrine trigger trips); allocation state (deployed/pending/dry powder); scenario positioning; recent earnings protocol triggers.

### Phase 2. Load vault context relevant to target (Glob + Grep)
...
### Phase 2.5. DELEGATED dispatch to `source-aggregator` subagent (PREFERRED path; Phase C wiring 2026-05-02)

**MANDATORY when an Agent tool exists** (per Execution Rules): dispatch first, no pre-emptive skip. Subagent handles N/A returns gracefully (e.g., topic with sparse external coverage -> empty ranked table + "low candidate corpus" synthesis + confidence: LOW; that is a SUCCESSFUL dispatch).

Use the Agent tool with subagent_type `source-aggregator`. Pass input: `{research_target: "<target slug + framing from Phase 1>", primary_topic_tags: [<from Phase 3 derivation if available, else inferred>], existing_vault_refs: [<list from Phase 2>]}`. The subagent searches preferred-domain whitelist (sec.gov, FRED, dataroma, PubMed, Cochrane, claude.com/docs, etc.) and skips blocked domains (Bloomberg paywalled, WSJ paywalled, Investopedia, Seeking Alpha, etc.) per ref-evidence-hierarchy.md, dedupes against existing Atlas/sources/ refs, and returns a ranked source candidate table + 50-80 word abstracts per candidate + vault-duplicate flags + blocked-domain skip summary.
...
**Documented inline fallback** (verbatim; fires on contract violation, dispatch failure, or a harness with no Agent tool):
...
### Phase 3. Derive canonical slug + target_path + frontmatter fields

From target:
- `slug` = lowercase-kebab-case of primary target (e.g., `theme-alpha-deep-dive`, `iran-kinetic-allocation`)
- `domain` = investing | tech | meta
...
### Phase 4. Compose prompt using template
...
### Phase 5. Write to wiki/research/prompts/`<slug>`-`<date>`.md
...
### Phase 6. Print workflow reminder (4 steps) + subagent dispatch report
...
- `Step 2.5 source-aggregator: FALLBACK (<reason>)` -- legitimate dispatch failure, contract violation, or no Agent tool in this harness; inline WebSearch+WebFetch source discovery during Phase 4
...
<Portfolio context block, auto-composed from Phase 1>

- Combined portfolio: ~$`<amount>` (per covered account, current-session broker read)
- Current positions (deployed): `<list>`
- Pending orders / limits: <list with triggers>
- Dry powder: $`<amount>`
- Active thesis exposure: <per-thesis % + doctrine status>
- Scenario positioning: <framing + time-conditional triggers>
- Recent earnings protocol triggers: <upcoming catalysts>

## EXISTING VAULT CONTEXT (your output should COMPLEMENT, not duplicate)

<Peer vault files block from Phase 2 -- list peer refs, thesis essays, analyses, entity notes with scope notes>
...
<Auto-composed per domain -- see /deep Process Phase 2 domain mapping>
...
## Discipline and interoperability

### Quality Rules

- Template markers `=== BEGIN RESEARCH MODE PROMPT ===` / `=== END RESEARCH MODE PROMPT ===` are byte-exact. Downstream copy-paste parsing depends on them.
- The composed prompt is the deliverable. /deep NEVER performs the research itself and never fabricates research findings in place of the Research-mode run.
- Writes exactly one file per run: `wiki/research/prompts/<slug>-<date>.md`. Same-day collision takes a `-HHMM` suffix (archival rule; dated outputs are never overwritten). `--preview` writes nothing.
- Path-guards are mechanical: `private/`, `.raw/`, `finance/`, `credentials/` are read-only or blocked; `Atlas/` is human-write-only, so /deep only NAMES the `target_path` it intends and never writes there.
- F11 Phase C discipline: set the flag before Write; single atomic commit; F14 narrow staging; F17 Co-Authored-By verify.
- ASCII only in all emitted content (Pattern 22): `--`, `->`, `>=`, `<=`, straight quotes.
- Every quantitative line injected into the CONTEXT block carries its provenance; portfolio share counts come from the broker read MCP at run time, never from `private/` files.
- The Phase 6 dispatch report is mandatory and states exactly one of DISPATCHED / FALLBACK / DEVIATION with a reason.
- Pre-output check: template markers present and byte-exact; `target_path` value identical in ROLE and in the frontmatter block; all `<placeholder>` fields filled from real context; ASCII clean; prompt-file frontmatter canonical.
...
### Example 1: /deep on AI infrastructure

`/deep theme-alpha-deep-dive`

Loads positions, active thesis, peer refs, analyses, entity notes. Composes prompt with `target_path = Atlas/sources/investing/ref-theme-alpha-deep-dive.md`. Research mode output emits that `target_path` in frontmatter. /enrich v9 reads it, places at exact path. /ingest distills 80+ claims across 7 entity notes.
