...
def main() -> int:
    archive_dir = Path(os.environ.get("PRECOMPACT_ARCHIVE_DIR", "/path/to/vault/.claude/transcripts"))  # env override for hermetic testing only; default unchanged
    ...
    hot = Path(os.environ.get("PRECOMPACT_HOT_PATH", "/path/to/vault/wiki/hot.md"))  # env override for hermetic testing only; default unchanged
    if hot.exists():
        snapshot_path = archive_dir / f"hot-snapshot-{stamp}.md"
        shutil.copy2(hot, snapshot_path)

    # X23 mission re-anchor (TENFOLD T2): snapshot the arc mission anchor so
    # post-compaction reconstruction recovers mission identity, not just state.
    arc = Path(os.environ.get("PRECOMPACT_ARC_PATH", "/path/to/vault/wiki/research/tenfold/arc-state.md"))
    if arc.exists():
        shutil.copy2(arc, archive_dir / f"arc-state-snapshot-{stamp}.md")
