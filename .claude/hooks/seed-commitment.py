...
# Negative lookahead excludes decision-log.md (the append-only LOG, not a decision
# record) -- without it every log append phantom-seeded a commitment (X05 fix, tenfold-t1).
DECISION_GLOB_RE = re.compile(r"Calendar[\\/]decisions[\\/]decision-(?!log\.md$)")
# S10 phantom-seed guard (tenfold-t10): never mint a review reminder from a demo/test
# decision record or a degenerate title. The decision-log.md source was already
# excluded by X05 (tenfold-t1 negative-lookahead above); this is defense-in-depth for
# any FUTURE demo/generic decision so it cannot regenerate the "Decision Log -- DEMO"
# class of perpetual open loop. Canonical decision-review tracking lives in
# open-loops.py scan_review_dates (reads decision-log.md review dates); this hook is
# only the tier-calibrated fallback for decisions lacking an explicit review_date.
GENERIC_TITLE_STOPLIST = {"decision log", "decisions", "decision", "untitled", ""}
# Unambiguous non-operational markers only. "test" is deliberately EXCLUDED -- a real
# decision can legitimately be about a test (A/B test, test protocol), and it collided
# with the prevention-arch harness's own decision-test-*.md fixture (T27). demo/DEMO
# (the S10 phantom-seed class) + the generic-title stoplist still catch the real cases.
DEMO_SLUG_RE = re.compile(r"\b(demo|placeholder|sample|dummy|scratch)\b", re.IGNORECASE)
DEMO_TITLE_RE = re.compile(r"\b(demo|placeholder|sample|dummy|scratch)\b", re.IGNORECASE)
TIER_DAYS = {
    "investing": 3,
    "career": 7,
    "skill-infra": 14,
...
def main() -> int:
    ...
    # S10 phantom-seed guard (tenfold-t10): a demo/test decision record or a degenerate
    # title yields a meaningless perpetual open loop -- skip it at the source, BEFORE any
    # side effect (so a skipped decision never even creates an empty daily note).
    title_norm = decision_text.strip().lower()
    if (title_norm in GENERIC_TITLE_STOPLIST
            or DEMO_TITLE_RE.search(decision_text)
            or DEMO_SLUG_RE.search(decision_slug)):
        sys.stderr.write(
            f"seed-commitment: SKIP demo/generic decision '{decision_slug}' "
            f"(title='{decision_text}') -- no phantom review reminder seeded.\n"
        )
        return 0
