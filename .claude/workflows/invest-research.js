...
if (!A.ticker) return { error: 'ticker required (args: {ticker, class_hint, held, entity_exists, ws_budget_map, run_timestamp, token_budget, vault_context_summary, prior_entity_summary, include_technicals, correlation_basket})' }
...
// v5 (invest-enhancement-pass 2026-07-11): deterministic technicals panel via
// price-fetcher include_technicals flag (crypto has no equity historicals -> off).
const INCL_TECH = A.include_technicals === true && CLASS_HINT !== 'crypto'
const BASKET = Array.isArray(A.correlation_basket) ? A.correlation_basket : []
...
const PRICE_SCHEMA = SKEL({
  ...
  technicals: { type: ['object', 'null'] },
  correlation_matrix: { type: ['object', 'null'] },
}, INCL_TECH ? ['quotes', 'failures', 'technicals'] : ['quotes', 'failures'])
...
const COMPETITIVE_SCHEMA = SKEL({
  ...
  sector_tailwinds: ARR, sector_headwinds: ARR,
...
// Production workers inherit the authorized session settings. Explicit caller
// options remain authoritative; fixed evaluation configurations have other owners.
...
const price = await dispatch('quote-technicals', 'Price',
  `Fetch quotes per your standard contract. Input: {"equities": ${JSON.stringify(CLASS_HINT === 'crypto' ? [] : [T])}, "crypto": ${JSON.stringify(CLASS_HINT === 'crypto' ? [T] : [])}, "include_technicals": ${JSON.stringify(INCL_TECH)}${INCL_TECH ? `, "correlation_basket": ${JSON.stringify(BASKET)}` : ''}}. Return your full JSON output contract (quotes, failures, extended_hours_movers, mcp_price_count${INCL_TECH ? ', technicals, correlation_matrix -- your v5 flag-gated deterministic panel via get_equity_historicals + tools/technicals.py; a degraded panel returns technicals:null + a failures[] entry, never a silent omit' : ''}). The parent gate requires broker_authoritative=true on held-equity quotes during regular session -- follow your v4 tier ladder exactly (Step 1 ToolSearch MCP load; load failure goes to failures[], never a silent skip).`,
  { agentType: 'price-fetcher', schema: PRICE_SCHEMA }, INCL_TECH ? ['quotes', 'failures', 'technicals'] : ['quotes', 'failures'])
...
return {
  ...
  epilogue: 'MAIN LOOP MUST now run the verdict spine: J portfolio-fit (hybrid live positions; regular_market_close anchoring) -> J.5 prior-research comparison -> J-bis integration (positioning bundle) -> K-bis.2 framework rotation -> K-bis.3 composite -> K-bis.4/5 verdict (spine VERBATIM) -> K.5 thesis-critic (Agent tool) -> Wave-3 gate check (invest-verify.js for every material BUY, HOLD or SELL / thesis-status change / boundary composite / --verify) -> K compose -> L entity -> M peripherals -> O commit.',
}
