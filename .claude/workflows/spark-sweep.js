...
// ---------- defensive args parse (regenerate-distillate.js precedent) ----------
let A = args
...
function CORPUS(cls) {
  ...
  if (cls === 'negative-space' || cls === 'meta') {
    if (EX('partw_extract')) blocks.push(`HONEST-GAPS REGISTER (master-context Part W; EXCLUDE/context list -- documented doc-only fictions, excised subsystems, and known-dormant domains are NOT mysterious absences; cite pointer-style as "(master-context Part W register)", never quote at length):\n${EX('partw_extract')}`)
  }
  // vNEXT2 2026-07-09 (skill-sota-pass-5): post-kernel organ extracts
  if (cls === 'behavioral' || cls === 'decision-divergence') {
    if (EX('gates_extract')) blocks.push(`JUDGMENT-GATES REGISTRY + IN-WINDOW SHEETS (wiki/research/gates/; verdict/mandate/outcome tuples + Compliance blocks -- FOMO-SUSPECT clusters, ROUTING-AROUND flags, and mandate follow-through gaps ARE ${cls} signal):\n${EX('gates_extract')}`)
  }
  if (cls === 'failure-mode' || cls === 'negative-space') {
    if (EX('eod_ledger')) blocks.push(`EXECUTE-OR-DECLINE LEDGER (Calendar/decisions/execute-or-decline.md; overdue PENDING rows, decline-vs-execute shapes, escalation ages -- ratified-but-unexecuted recurrence IS ${cls} signal):\n${EX('eod_ledger')}`)
  }
  if (cls === 'thesis-evolution' || cls === 'decision-divergence') {
    if (EX('kernel_journal_rows')) blocks.push(`INVEST-KERNEL JOURNAL ROWS (in-window analyses kernel frontmatter: rating/confidence/conviction/scoring_path/deployment_override -- rating-vs-action divergence, confidence drift, and override usage ARE ${cls} signal):\n${EX('kernel_journal_rows')}`)
  }
  if (cls === 'meta') {
    if (EX('projects_index')) blocks.push(`PROJECTS INDEX (wiki/projects/projects-index.md; genesis-skeleton instantiation record -- build-vs-consume asymmetry context):\n${EX('projects_index')}`)
  }
...
HARD RULES: Read/Grep/Glob only (you have no other tools). Do NOT read <private-file>*.local.md or any *.local.md master-context file -- parent-passed extracts below are your only window into it. ASCII-only output. Evidence quotes must be literal source lines.${retroRule}
...
return {
  ...
  epilogue: 'MAIN LOOP MUST now run: Phase E.1 inline fallback (per ref-pattern-taxonomy.md) for fallback_classes ONLY -> Phase D continuity merge (+ prediction scoring) -> F substantive filter (+ playbooks dedup) -> G promotion cap -> H confidence calibration -> H.5 verify gate (spark-verify.js when any promoted spark qualifies) -> I meta-observation -> J FOLLOWUPS -> K 22-item gate -> L ASCII scan -> M compose -> N verify -> O atomic write -> P peripherals.',
}
