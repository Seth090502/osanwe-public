...
execution failures, and the routing fixtures that record their real prompts. `AUDIT.md` explains how they were
found.
...
Each directory keeps a stub naming its generator, its source and the command that recreates it. Two
hand-written files inside `.claude/skills/` are on the generator's never-touch list and are published.
