#!/usr/bin/env python3
"""
auto-count-sync.py -- PostToolUse hook firing on AGENTS.md + Atlas/_MOCs/* writes.
...
Trigger:
  PostToolUse Write|Edit|MultiEdit on file_path matching:
    - AGENTS.md (the root contract; a legacy CLAUDE.md still matches)
    - Atlas/_MOCs/*.md
...
def is_trigger_path(rel_str: str) -> bool:
    """Return True if rel_str is the root contract or under Atlas/_MOCs/."""
    if rel_str in ("AGENTS.md", "CLAUDE.md"):
        return True
