...
# Used for bulk migrations (e.g., Group 25 frontmatter) where per-file auto-commit
# stream would overwhelm the atomic-commit discipline.

[ "${CLAUDE_DISABLE_AUTO_COMMIT:-}" = "1" ] && exit 0
[ -f /path/to/vault/.claude/state/auto-commit-disabled ] && exit 0
...
case "$file_path" in
    /path/to/vault/*|/path/to/vault/*|/path/to/vault\\*) ;;
    *) exit 0 ;;
esac

cd /path/to/vault 2>/dev/null || exit 0
...
if ! git diff --quiet "$file_path" 2>/dev/null || [ -n "$(git ls-files --others --exclude-standard "$file_path" 2>/dev/null)" ]; then
    ...
    rel_path=$(echo "$file_path" | sed 's|^/path/to/vault/||' | sed 's|^/path/to/vault/||')

    case "$file_path" in
        *.md|*.base)
            # Only audit markdown/base files; other files (json, py, sh) skip
            if [ "${CLAUDE_VAULT_BYPASS_VALIDATOR:-}" != "1" ]; then
                gate_count=$(python tools/vault-audit.py --scope "$rel_path" --json 2>/dev/null | python -c "
import json, sys
try:
    d = json.load(sys.stdin)
...
" 2>/dev/null || echo "0")
                if [ "${gate_count:-0}" -gt 0 ]; then
                    ...
                    # Log to bypasses log for retrospective audit
                    log_dir="/path/to/vault/.claude/state"
    ...
    # X30 mirror-staging REMOVED 2026-08-10 (cross-harness migration S2 disarm):
    # the .agents/skills pointer-adapter generator is deregistered + refused, so
    # .claude/skills writes no longer dirty a sibling mirror. Staging direction
    # inverts at S6 cutover (canon .agents/skills -> derived .claude/skills).
    git commit -m "agent: $verb $rel_path" --no-verify >/dev/null 2>&1 || true
    # X30 detection line (no standing sweeper per red-team): surface residue, never mutate.
    residue=$(git status --porcelain 2>/dev/null | wc -l || echo 0)
    if [ "${residue:-0}" -gt 5 ]; then
        echo "auto-commit: WARN ${residue} dirty paths remain post-commit (F11 residue / script-write class X30) -- consider a characterized sweep." >&2
    fi
fi
