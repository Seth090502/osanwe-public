...
Source pattern: AgriciDaniel/claude-obsidian@75d3b6feb77b96c6bb16599c4550cc9703553d87
  ...
  Adapted from "prompt"-type to "command"-type to match Osanwe's
...
# testing (T29) and never change runtime behavior when unset.
...
def main() -> int:
    ...
    # X23 mission re-anchor (TENFOLD T2): emit the arc CONTINUATION PROTOCOL
    # section so a compacted session recovers its mission identity (the
    # observed failure: 16 forks lost role/mission across compaction).
    try:
        arc = Path(os.environ.get("POSTCOMPACT_ARC_PATH", str(VAULT_ROOT / "wiki" / "research" / "tenfold" / "arc-state.md")))
        if arc.exists():
            m = re.search(r"(## CONTINUATION PROTOCOL.*?)\n## ", arc.read_text(encoding="utf-8"), re.DOTALL)
            if m:
                sys.stdout.write("[mission re-anchor (X23)]\n" + m.group(1).strip()[:1500] + "\n\n")
    except Exception:
        pass
